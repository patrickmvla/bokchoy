## Current state
- **Mode:** implementation → slice 8.3.5 (Suspense investigation + sign-up surface) LANDED 2026-05-13. Two related concerns from prior seat's "next on resume" queue. **Empirical proof**: `/sign-up` HTTP 200 (29803 bytes; all UI markers present), `/sign-in` HTTP 200 with new cross-link, `/projects` HTTP 307 (auth gate clean). Three gates: typecheck ✓ (6 packages), cockpit lint ✓ (48 files; +3 net for sign-up + cross-link), cockpit dev ✓.
- **Suspense investigation outcome — wrap REJECTED, layout-gate kept:**
  - Hypothesis was that wrapping `getSession()` in `<Suspense>` would silence the Cache Components warning. Empirically verified the wrap silences the warning BUT regresses auth-gate semantics: HTTP 307 collapses to HTTP 200 with embedded RSC redirect markers. Backend data does NOT leak (all fetches client-side, cookie-gated), but static UI strings ("New project" CTA label) appear in the response body via RSC streaming format.
  - Trade-off accepted: keep the 307 + dev warning. Crawlers, monitoring, and non-browser clients get clean redirect semantics; the dev warning is documented in `app/(app)/layout.tsx` as a known trade-off. **Owed migration** documented in the layout file's header comment: move the gate to `middleware.ts` for an edge-level optimistic cookie check (`getSessionCookie` per `[[cockpit-stack-integration-research]]` F6.5) — zero leak, no warning, real 307 at the edge. Layout's `getSession` stays as authoritative defense-in-depth for expired/invalid cookies. Tracked as a follow-on slice.
- **Sign-up surface LANDED:**
  - `apps/cockpit/modules/auth/lib/sign-up-schema.ts` (NEW) — Zod schema: `name` min(1) max(100), `email` z.email(), `password` min(8) max(128). Constraints aligned with Better Auth's `emailAndPassword` defaults (minPasswordLength=8 per Better Auth v1.x server validation).
  - `apps/cockpit/modules/auth/components/sign-up-form.tsx` (NEW) — Client Component mirroring `<SignInForm>` shape. OAuth section identical (Google + GitHub via `authClient.signIn.social` — auto-provisioning per [[cockpit/first-run-journey]] step 3). Email/password section adds `name` field above email; uses `authClient.signUp.email({email, password, name})`. Same show/hide password toggle pattern as sign-in (inlined, NOT yet extracted to a shared `<PasswordInput>` — third use will trigger extraction at the settings password-change slice). On success → `router.push('/projects')` (Better Auth auto-signs-in on signUp.email). Same try/catch/userMessage error handling pattern.
  - `apps/cockpit/app/sign-up/page.tsx` (NEW) — Server Component shell. Outside (app)/ route group — no auth gate. Cross-link footer "Already have an account? Sign in" → /sign-in.
  - `apps/cockpit/app/sign-in/page.tsx` (EDIT) — added cross-link footer "New to BokChoy? Create an account" → /sign-up.
  - `apps/cockpit/modules/auth/components/sign-in-form.tsx` (EDIT) — removed the inline OAuth-only-provisioning footer hint (superseded by the /sign-up cross-link on the page).
- **Email verification NOT in this slice.** Better Auth's `emailAndPassword.requireEmailVerification` defaults to false; new accounts can sign in immediately. Production-readiness owed: enable verification + ship `/verify-email` surface. Tracked as a follow-on slice. Not blocking MVP.
- **Password toggle extraction deferred.** Two consumers (sign-in + sign-up) now duplicate the same ~30-line `<div className="relative">` + `<Input className="pr-10">` + absolute-positioned toggle button. Extraction trigger: third consumer (settings password-change). Cost of extraction now would be a new `<PasswordInput>` component with `forwardRef` for RHF compatibility — net-zero LOC but adds a forwardRef indirection that's wasted at 2 consumers.
- **Slice 8.3 obligations REMAINING:**
  - **Email verification surface + flow** (deferred).
  - **Middleware-level auth gate** (Suspense investigation outcome — owed for clean Cache Components story).
  - **Empirical OAuth flow test** — operator-driven, still owed.
  - **Smoke-script empirical run** for slice 8.4 — extend with DELETE + orgs coverage before running.
  - **Five vault amendments queued** — /design pass.
- **Next on resume — recommended sequence:**
  1. **/implementation slice 8.3.6 — middleware.ts auth gate**. Optimistic cookie check at the edge via `getSessionCookie` from better-auth/cookies; layout's `getSession` becomes authoritative defense-in-depth. Closes the Cache Components warning + the (small) UI-string leak on auth-fail.
  2. **/implementation — sign-out chrome enhancements** (none owed currently; AppHeader already has it).
  3. **/implementation — smoke-script empirical run** (operator-driven).
  4. **/design pass** — five contract amendments + RSC migration plan.

- **Mode (prior):** implementation → slice 8.3 (GET /v1/orgs/me + auth chrome) LANDED 2026-05-13. Backend handler returns the active org + member from request-local adminGate context (zero extra DB hits — the rows are already set on c.var via Step 4/4b). Cockpit chrome (AppHeader) mounted in (app)/layout.tsx renders org name + role badge + user email + sign-out button across all gated routes. 3 gates green: monorepo `bun run typecheck` ✓ (6 packages), cockpit `bun run lint` ✓ (45 files; +6 net for orgs module + auth module), cockpit `bun run dev` ✓ (Ready in 2.3s, Cache Components enabled).
- **Files landed this seat (5 new + 2 edits):**
  - `apps/backend/src/orgs/index.ts` (NEW) — `getOrgMeHandler` returning `{id, name, slug, createdAt, member: {id, userId, role, createdAt}}` bare per (R2). No DB query — adminGate Step 4b already SELECTed the org row + Step 4 SELECTed the member row, both set on c.var. OTel span `orgs.get_me` with `bokchoy.organization_id` + `bokchoy.user_id` + `bokchoy.role` attrs. `mountOrgsRoutes(app: Hono<OrgsAppContext>)` mount function — `OrgsAppContext = ApiKeyContext & IdempotencyContext & AdminContext` per Hono generic invariance (caught at typecheck on first try; same pattern projects + wallet modules use).
  - `apps/backend/src/index.ts` (EDIT) — imported `mountOrgsRoutes` and mounted between Better Auth handler + projects routes (alphabetical-ish; not load-bearing).
  - `apps/cockpit/modules/auth/types.ts` (NEW) — `MemberRole`, `OrgMember`, `OrgMe` types matching backend handler response.
  - `apps/cockpit/modules/auth/api/get-org-me.ts` (NEW) — fetch wrapper through Vercel rewrite. Throws `OrgApiError` with `.code` + `.status` on non-2xx. Distinct from `ProjectApiError` to keep modules decoupled — both share the same wire-shape (Stripe-wrapped error envelope per G5).
  - `apps/cockpit/modules/auth/hooks/use-org-me.ts` (NEW) — TanStack `useQuery` with `queryKey: ['org-me']`, `staleTime: 5min`, `refetchOnWindowFocus: false`. Long stale-time because org info changes rarely.
  - `apps/cockpit/modules/auth/components/app-header.tsx` (NEW) — Client Component. Layout: BokChoy wordmark + " / " + org name + role badge (left), user email + Sign-out button (right). `userEmail` is a prop from the layout (already in `getSession()` result — no double-fetch). `handleSignOut` calls `authClient.signOut()`, clears the TanStack Query cache (`queryClient.clear()`) to prevent stale-data flash on next sign-in, then `router.push('/sign-in')`. try/catch + `userMessage(err)` for failure toast. LogOutIcon from lucide-react during idle; Spinner during pending state.
  - `apps/cockpit/app/(app)/layout.tsx` (EDIT) — mounted `<AppHeader userEmail={session.user.email} />` before children. Children wrapped in `<div>` (placeholder; could become a main-content wrapper with consistent padding later).
- **Pre-write attack notes vaulted in code comments:**
  - **Zero-DB-query handler.** adminGate already SELECTs the org + member rows before the handler runs — re-querying would be wasteful. Handler is a thin response shaper.
  - **OrgApiError vs ProjectApiError.** Two distinct typed error classes (rather than a shared `BackendApiError`). Keeps modules decoupled — if either's wire shape evolves, the other doesn't ripple. Both compose with `userMessage()` via duck-typing on `.code`.
  - **`queryClient.clear()` on sign-out.** Without it, after sign-in the new user would briefly see the previous user's cached `useProjects` / `useOrgMe` data — including potentially cross-org data leakage if accounts are linked to different orgs. Clearing is a security defense, not just UX polish.
- **What the user actually sees now:**
  - Sign in → land on /projects → chrome header shows `BokChoy / Acme Inc admin   alice@example.com [Sign out]` across all (app)/ routes.
  - Click Sign out → cookie cleared, query cache cleared, redirected to /sign-in.
  - Network error during sign-out → `toast.error("Network error. Check your connection and try again.")` + button unlocks.
- **Slice 8.3 obligations REMAINING:**
  - **Empirical OAuth flow test** — still owed (operator-set env + provider config + browser test).
  - **Sign-up surface** — explicit email/password sign-up; OAuth still provisions on first sign-in.
  - **Cache Components <Suspense> cleanup** at /projects — pre-existing from 8.3.2; the new AppHeader's useOrgMe fetch is client-side so doesn't worsen this. Still owed.
  - **Smoke-script empirical run** for slice 8.4 — extend with DELETE coverage + new orgs endpoint before running.
  - **Five vault amendments queued** (CL baseURL, P socialProviders, prefix→keyPrefix, error-code casing, first-run-journey terminology) — /design mode work.
- **Next on resume — recommended sequence:**
  1. **/implementation slice 8.3.5 — Cache Components <Suspense> cleanup**. Wrap the session-read in (app)/layout.tsx in <Suspense> (or move it into a server-action style guard). Clears the dev warning + aligns with the RSC migration owed for the projects + detail views.
  2. **/implementation — sign-up surface** (`app/sign-up/page.tsx` mirroring sign-in). Closes the OAuth-only-provisioning gap.
  3. **/implementation — smoke-script empirical run** (operator-driven).
  4. **/design pass** — five contract amendments.

- **Mode (prior):** implementation → form error-handling pass LANDED 2026-05-13. Six fixes across three forms (sign-in, create-project, revoke-api-key) + central `userMessage()` helper. Three gates green: typecheck ✓ (6 packages), cockpit lint ✓ (41 files; +1 for error-messages.ts), cockpit dev ✓ + **empirical proof**: `curl /sign-in` HTTP 200, `curl /projects` HTTP 307 (auth gate redirect fires correctly).
- **Files landed this seat (1 new + 4 edits):**
  - `apps/cockpit/lib/error-messages.ts` (NEW) — `userMessage(err: unknown): string` helper. Three-layer mapping: (1) known code via `KNOWN_CODES` table (PROJECT_SLUG_EXISTS, API_KEY_NOT_FOUND, ALREADY_REVOKED, VALIDATION_ERROR, BC401–BC405, Better Auth INVALID_EMAIL_OR_PASSWORD / USER_NOT_FOUND / EMAIL_NOT_VERIFIED / CREDENTIAL_ACCOUNT_NOT_FOUND), (2) network-class TypeError ("Failed to fetch") → "Network error. Check your connection.", (3) fall through to err.message or "Something went wrong." Duck-types on `.code` field (`hasCode()` type guard) rather than importing `ProjectApiError` — keeps lib/ as a leaf module without cross-module deps. **PROJECT_SLUG_EXISTS message now hints at the (E2) race-window**: "If you just submitted this form, your project may have been created — check the Projects list before retrying."
  - `apps/cockpit/modules/auth/components/sign-in-form.tsx` (EDIT) — **critical bug fix**: `onEmailSubmit` was missing try/catch; a thrown error (network drop, unexpected throw) left `emailPending=true` forever, locking the button on "Signing in…" with no user feedback. Wrapped in try/catch; on throw: `toast.error(userMessage(err))` + `setEmailPending(false)`. On success path: don't clear pending (stays "Signing in…" through `router.push` → unmount, avoids button-label flicker). Both `handleOAuth` and `onEmailSubmit` now consume `userMessage()` instead of inline `error.message ?? 'unknown error'` constructions — drops the "google sign-in failed: unknown error" useless-fallback.
  - `apps/cockpit/modules/projects/components/create-project-form.tsx` (EDIT) — replaced `toast.error(error.message ?? 'Failed to create project.')` + `toast.error(error.message ?? 'Retry failed. Try again.')` with `toast.error(userMessage(error))`. ProjectCreatedButKeyFailedError branch unchanged (its message is hand-curated and accurate).
  - `apps/cockpit/modules/projects/components/project-detail-view.tsx` (EDIT) — replaced `toast.error(\`Revoke failed: ${error.message}\`)` with `toast.error(userMessage(error))`. ALREADY_REVOKED branch unchanged (special-cased as benign race, neutral toast).
  - `apps/cockpit/modules/projects/lib/create-project-schema.ts` (EDIT) — slug regex message tightened from technical "Slug must be kebab-case (lowercase letters, numbers, and hyphens; no leading/trailing or consecutive hyphens)." to user-friendly "Use lowercase letters, numbers, and single hyphens (e.g. my-game). No spaces or special characters." Name min/max + slug min/max messages were already actionable.
- **What each form user actually sees now (verified vs walkthrough):**
  - **Sign-in OAuth**: provider not configured / trustedOrigins mismatch → toast with friendly message (was: "<provider> sign-in failed: unknown error"). Network drop pre-redirect → "Network error. Check your connection." (was: technical TypeError leak).
  - **Sign-in email/password**: throw → toast.error + button unlocks (was: button locked forever).
  - **Create project**: 409 slug-exists → toast with race-window hint (was: bare backend message). Network error → "Network error" toast (was: "TypeError: Failed to fetch").
  - **Revoke key**: 404 → "This API key doesn't exist anymore. Refresh the page and try again." (was: "Revoke failed: API key not found under this project."). 422 already-revoked → unchanged benign-race neutral toast.
- **Out-of-scope finding (deferred):** **Cache Components warning at `/projects`** — `getSession()`'s uncached `fetch` in `(app)/layout.tsx` triggers Next.js's "Uncached data or connection() was accessed outside of <Suspense>" warning. Non-blocking; the redirect still fires (307 → /sign-in). Pre-existing from slice 8.3.2's gate landing. Already implicit in the **RSC migration** obligation already on the queue. Fix: wrap layout's session-dependent rendering in `<Suspense>`, OR mark layout dynamic, OR move session-read into a server-action-style guard. Defer to a dedicated RSC-cleanup seat.

- **Mode (prior):** implementation → slice 8.3.3 polish (OAuth brand icons on sign-in buttons) LANDED 2026-05-13. Inline SVGs for Google "G" multi-color mark (4 paths, official `#4285F4`/`#34A853`/`#FBBC05`/`#EA4335`) + GitHub Octocat (monochrome `currentColor`, adapts to button color). Lucide-react 1.14.0 has 1952 icons but excludes trademarked brand marks per library policy — falsified the assumption that `lucide-react` ships these icons. Cockpit `bun run lint` ✓ (40 files; +1 for oauth-icons.tsx). Empirical proof: `curl /sign-in` HTML contains `#4285F4` + `#34A853` + both button labels with HTTP 200. Sources: Google brand guidelines page (developers.google.com/identity/branding-guidelines), github.com/logos.
- **Files landed this seat (1 new + 1 edit):**
  - `apps/cockpit/modules/auth/components/oauth-icons.tsx` (NEW) — `<GoogleIcon>` + `<GithubIcon>` SVG components. `SVGProps<SVGSVGElement>` typed; aria-label set for screen readers; GitHub uses `fill="currentColor"` so it picks up button text color (works light + dark). Compliance note in header: official Google "Sign in with Google" widget is the strict-compliance path; inline-icon-on-our-own-button is the B2B SaaS norm and what we ship for MVP.
  - `apps/cockpit/modules/auth/components/sign-in-form.tsx` (EDIT) — replaced empty-when-not-pending button content with `<GoogleIcon className="mr-2 size-4" />` / `<GithubIcon className="mr-2 size-4" />`. Spinner takes the icon slot during the OAuth-pending state (same `mr-2 size-4`).
- **Slice 8.3.3 obligations REMAINING (unchanged from prior seats — icons are polish, not a contract):**
  - **Empirical OAuth flow test** — neither Google nor GitHub OAuth verified end-to-end. Requires operator-set env vars + provider redirect URI registration.
  - **Sign-up surface** — explicit email/password sign-up route is a follow-on slice.
  - **Sign-out + auth chrome** — header / user menu with sign-out trigger.

- **Mode (prior):** implementation → slice 8.3.3 (sign-in page + Better Auth client + socialProviders config) LANDED 2026-05-13. Contracts satisfied: F7 OAuth-primary sign-in pattern per `[[cockpit-stack-integration-research]]` + (CL) Better Auth React client + (P) socialProviders shape per `[[cockpit/auth-surface-mount]]`. Closes the 404 on `/sign-in` left by slice 8.3.2 — auth-gate redirect now lands on a functional sign-in page. 3 gates green + **empirical proof**: monorepo `bun run typecheck` ✓ (6 packages), cockpit `bun run lint` ✓ (39 files; +4 net), cockpit `bun run dev` ✓ (Ready in 3.2s, Cache Components enabled) AND `curl http://localhost:3001/sign-in` → HTTP 200 (24555 bytes) with `GET /sign-in 200 in 15.5s` in the trace (first-compile Turbopack cold; subsequent loads cached).
- **Files landed this seat (5 new + 2 edits):**
  - `packages/auth-config/src/index.ts` (EDIT) — added `buildSocialProviders()` helper + `socialProviders: buildSocialProviders()` to the `createAuth` factory. Conditional registration (Google + GitHub registered only when both `*_CLIENT_ID` + `*_CLIENT_SECRET` env vars are set). Local dev without OAuth keys still boots — falsified the contract's literal `process.env.X!` shape because that would have crashed the auth instance init at boot with undefined values. Conditional shape preserves the contract intent ("values via env") without blocking dev.
  - `apps/cockpit/package.json` (EDIT) — added `"better-auth": "catalog:"` dep (catalog resolves to ^1.4.0). `bun install` materialized 28 packages.
  - `apps/cockpit/lib/auth-client.ts` (NEW) — `createAuthClient` per (CL) + organizationClient plugin + re-exports of signIn/signOut/useSession/organization. **Contract amendment landed inline**: the vault's `baseURL: '/'` (CL line 122) rejects at module-eval with `BetterAuthError: Invalid base URL: /` — Better Auth parses baseURL with `new URL()` which requires an absolute URL. Replaced with `resolveBaseURL()` helper: browser uses `window.location.origin`; SSR fallback is `http://localhost:3001`. Same-origin requests through Vercel rewrites still hold; just the syntactic form of "same-origin" changed. Amendment to `[[cockpit/auth-surface-mount]]` (CL) owed.
  - `apps/cockpit/modules/auth/lib/sign-in-schema.ts` (NEW) — Zod v4 schema: `email` (z.email format) + `password` (min(1)). Client-side validation is minimal — Better Auth's backend handler does the real check.
  - `apps/cockpit/modules/auth/components/sign-in-form.tsx` (NEW) — Client Component. OAuth-primary: two `<Button variant="outline" className="w-full">` triggers calling `authClient.signIn.social({provider, callbackURL: '/projects'})`. Email/password fallback: RHF `useForm` + `zodResolver` + shadcn `<Field>` + `<Controller>` per `[[feedback-shadcn-form-field-pattern]]` → `authClient.signIn.email`. Spinner during pending state on each button independently. Separator with "Or continue with email" between OAuth and email blocks. Footer line: "New to BokChoy? Sign in with Google or GitHub above — your account is created on first sign-in" (OAuth auto-provisions per [[cockpit/first-run-journey]] step 3 + (O1)).
  - `apps/cockpit/app/sign-in/page.tsx` (NEW) — Server Component shell rendering `<SignInForm>`. OUTSIDE the `(app)/` route group — no auth gate (public-reachable by design). Centered max-w-md layout.
- **Empirical findings (NOT in vault before this seat):**
  - **(CL) `baseURL: '/'` broken** — flagged above. Amendment block owed to `[[cockpit/auth-surface-mount]]`.
  - **`socialProviders` config from (P) needs conditional shape** for local dev — env-driven literal `process.env.X!` crashes when vars are unset. The conditional `buildSocialProviders()` preserves the contract intent. Amendment to `[[cockpit/auth-surface-mount]]` (P) owed.
- **Slice 8.3.3 obligations REMAINING (not blocking; tracked):**
  - **Empirical OAuth flow test** — neither Google nor GitHub OAuth has been verified end-to-end. Requires operator to set `GOOGLE_CLIENT_ID/SECRET` + `GITHUB_CLIENT_ID/SECRET` env vars, register the cockpit redirect URIs at the provider consoles, and exercise the full sign-in → callback → /projects flow. Safari ITP production verification per `[[cockpit-stack-integration-research]]` open thread #2 also owed.
  - **Sign-up surface** — current sign-in page accepts existing email/password but has no sign-up route. OAuth auto-provisions on first sign-in (Better Auth default behavior). Explicit email/password sign-up is a follow-on slice (likely `app/sign-up/page.tsx` mirroring sign-in-form.tsx).
  - **Sign-out surface** — `signOut` is exported from auth-client but no UI exposes it yet. Owed when the cockpit chrome (header / user menu) lands.
- **Slice 8.4 obligations STILL REMAINING (unchanged from prior seats):**
  - **Smoke-script empirical run** (`/tmp/smoke-8-4-projects-admin.sh`) — still UNVERIFIED at runtime. K1 (RLS-INSERT-on-api-keys) is the load-bearing test. Extend with DELETE coverage before running, OR run as-is then extend.
  - **`GET /v1/orgs/me`** org-info backend endpoint (slice 8.3 cascade).
  - **`[[cockpit/admin-list-endpoints-contract]]` amendments** (`prefix`→`keyPrefix`, error-code casing).
  - **`[[cockpit/first-run-journey]]` step 5 + step 6 terminology amendments**.
  - **`[[cockpit/auth-surface-mount]]` (CL) baseURL + (P) socialProviders shape amendments** (NEW from this seat).
  - **RSC migration for `<ProjectDetailView>` + `<ProjectsList>`**.
  - **API-key issuance UI on detail page** — read + revoke landed; issue-additional-key UI owed.
- **Next on resume — recommended sequence:**
  1. **/implementation slice 8.3 — `GET /v1/orgs/me`** backend endpoint + cockpit org-display chrome (header / user menu surface) — natural pair with sign-in since the chrome needs to know the user's active org. Smaller scope; one handler + one cockpit component.
  2. **/implementation slice 8.3.4 — sign-out + auth chrome** — cockpit header with user menu, sign-out trigger, active-org display. Builds on (1).
  3. **/implementation — smoke-script empirical run** (operator-driven; extend with DELETE coverage first).
  4. **/design pass** — five contract amendments queued (`prefix`→`keyPrefix`, error-code casing, first-run-journey terminology, (CL) baseURL, (P) socialProviders) + RSC migration plan for `<ProjectDetailView>`/`<ProjectsList>`.

- **Mode (prior):** implementation → slice 8.3.2 (auth gate) LANDED 2026-05-13. Contract satisfied: RSC session-read primitive (`lib/session.ts`) + auth-gated route group layout (`app/(app)/layout.tsx`) per `[[cockpit-stack-integration-research]]` F5.5 + F5.6 + cascade obligations #5+#6. Closes the public-reachability hole on `/projects`, `/projects/new`, `/projects/{id}` flagged through 3 prior seats. 3 verification gates green: monorepo `bun run typecheck` ✓ (6 packages), cockpit `bun run lint` ✓ (35 files; +2 net; zero errors / zero warnings), cockpit `bun run dev` ✓ (Ready in 1440ms on port 3001, Cache Components enabled; `GET /sign-in 404` captured — the gate's redirect fires; the destination page is the next slice).
- **Files landed this seat (2 new):**
  - `apps/cockpit/lib/session.ts` (NEW) — `getSession()` wrapped in `React.cache`. Forwards `cookie` header from Next.js `headers()` to backend's `/api/auth/get-session` via absolute URL constructed from `BOKCHOY_BACKEND_URL` (reuses existing env var — the research entry's `BOKCHOY_INTERNAL_URL` is a forward-compat optimization for production internal-network routing, NOT a deps requirement; flagged for production-deploy slice). `cache: 'no-store'` overrides Next.js 16's fetch default to keep each request's session check fresh. Inline `Session` type (`user: {id,email,name?,image?}` + `session: {id,activeOrganizationId?}`) — minimal shape sufficient for the gate's `if (!session)` check. Full Better Auth typing owed when cockpit adds the `better-auth` dep for the sign-in UI slice. Try/catch around fetch → null on network error: backend-down degrades into redirect-to-sign-in rather than crashing mid-stream.
  - `apps/cockpit/app/(app)/layout.tsx` (NEW) — Server Component (NO 'use client' per F6.11). Awaits `getSession()`; `redirect('/sign-in')` on null. Children passthrough. Same-request `React.cache` memoization in `getSession` means nested layouts/pages reading session reuse this lookup — no double fetch.
- **Pre-write attack notes vaulted in code comments:**
  - **`BOKCHOY_INTERNAL_URL` deferred to production-deploy slice.** Research entry suggested separating internal vs external URLs; for MVP local dev they're identical and reusing `BOKCHOY_BACKEND_URL` avoids env-var sprawl. When production splits the URLs (Railway internal DNS / Vercel internal proxy), add the override there.
  - **Session shape minimally typed.** Promote to Better Auth's full `Session` type when `better-auth` lands in cockpit deps (next slice for sign-in UI). Current shape covers the gate's "is the user logged in?" question + the most common downstream consumer fields (`user.id`, `user.email`, `session.activeOrganizationId`).
  - **Backend-down treated as no-session.** Conservative interpretation of `if (!session) redirect('/sign-in')` — any failure to validate (network error, malformed response, expired token) lands the operator at sign-in where the error is visible, rather than crashing inside a half-rendered RSC stream.
- **Slice 8.3.2 obligations REMAINING (not blocking; tracked for next slice):**
  - **`/sign-in` page** — currently 404. Slice owed: `apps/cockpit/app/sign-in/page.tsx` (OAuth-primary form per F7) + `apps/cockpit/lib/auth-client.ts` (Better Auth React client) + `better-auth` dep added to cockpit `package.json`. THIS slice intentionally lands the gate before the destination so the data-leak hole closes immediately; a 404 on /sign-in is worse UX than a working sign-in page, but a much better UX than exposing org data.
  - **Full Better Auth `Session` type import** owed once `better-auth` lands in cockpit deps — promote `lib/session.ts`'s inline interface to the typed import.
- **Slice 8.4 obligations STILL REMAINING (unchanged from prior seat):**
  - **Smoke-script empirical run** (`/tmp/smoke-8-4-projects-admin.sh`) — still UNVERIFIED at runtime. K1 (RLS-INSERT-on-api-keys) is the load-bearing test. Extend with DELETE coverage before running, OR run as-is then extend.
  - **`GET /v1/orgs/me`** org-info backend endpoint (slice 8.3 cascade).
  - **`[[cockpit/admin-list-endpoints-contract]]` amendments** (`prefix`→`keyPrefix`, error-code casing, lowercase→UPPER_CASE codes) owed in next /design pass.
  - **`[[cockpit/first-run-journey]]` step 5 + step 6 terminology amendments** owed in next /design pass.
  - **RSC migration for `<ProjectDetailView>` + `<ProjectsList>`** — first-run-journey step 4/11 specify RSC fetching. Current Client Component pattern works behaviorally; RSC migration is the next ergonomic step now that the session primitive exists.
  - **API-key issuance UI on detail page** — read + revoke landed; issue-additional-key UI owed.
- **Next on resume — recommended sequence:**
  1. **/implementation slice 8.3.3 — sign-in page** + `lib/auth-client.ts` + `better-auth` dep add. Closes the 404 on the redirect target. Per F7: OAuth-primary form with Google + GitHub buttons + email/password fallback.
  2. **/implementation slice 8.3 — GET /v1/orgs/me** backend endpoint + cockpit org-display chrome.
  3. **/implementation — smoke-script empirical run** (operator-driven; extend with DELETE coverage first).
  4. **/design pass** — three contract amendments queued (`prefix`→`keyPrefix`, error-code casing, first-run-journey terminology) + RSC migration plan for `<ProjectDetailView>`/`<ProjectsList>`.

- **Mode (prior):** implementation → slice 8.4 NEXT-CUT (`DELETE /v1/projects/{projectId}/api-keys/{keyId}` revoke handler + cockpit revoke UI) LANDED 2026-05-13. Contract satisfied: API-key soft-revoke per `[[cockpit/admin-list-endpoints-contract]]` (`revoked_at` sentinel; slice 8.1a `apiKeyMiddleware` already filters revoked rows at lookup, so a successful revoke immediately invalidates the key for SDK calls — no cache to bust). 3 verification gates green: monorepo `bun run typecheck` ✓ (6 packages), cockpit `bun run lint` ✓ (33 files; +2 net), cockpit `bun run dev` ✓ (Ready in 661ms on port 3001, Cache Components enabled, no warnings).
- **Files landed this seat (2 new + 2 edits):**
  - `apps/backend/src/projects/index.ts` (EDIT) — added `revokeApiKeyHandler` + new `revokeApiKeyUrlParams` zod schema. `isNull` added to drizzle-orm imports. UPDATE api_keys SET revoked_at = NOW() WHERE id = keyId AND project_id = projectId AND revoked_at IS NULL inside `withTenant(db, projectId)`. On 0-row result, follow-up tenant-scoped SELECT disambiguates 404 (`API_KEY_NOT_FOUND`) from 422 (`ALREADY_REVOKED`). OTel span `api_keys.revoke` with `bokchoy.revoke_outcome` attr (`revoked` / `not_found` / `already_revoked`). Mounted on `app.delete('/v1/projects/:projectId/api-keys/:keyId', adminGate({ resource: 'apiKey', actions: ['revoke'], projectIdParam: 'projectId' }), sValidator('param', revokeApiKeyUrlParams, validationFailureHook), revokeApiKeyHandler)`. Module header comment updated to "Five endpoints". `auth-config` already had `apiKey: ['create', 'revoke']` for adminRole + ownerRole; memberRole still has no apiKey actions (revoke → BC405).
  - `apps/cockpit/modules/projects/api/revoke-api-key.ts` (NEW) — DELETE fetch wrapper, reuses `ProjectApiError` from `create-project.ts`. Returns `{ id, revokedAt }`. Throws `ProjectApiError` with `.code` `'API_KEY_NOT_FOUND'` (404) or `'ALREADY_REVOKED'` (422) on non-2xx.
  - `apps/cockpit/modules/projects/hooks/use-revoke-api-key.ts` (NEW) — TanStack `useMutation`. onSuccess: `queryClient.invalidateQueries({ queryKey: ['project', projectId] })` so the detail view refetches and the UI flips immediately. Error branching is caller's responsibility (mutation surfaces `ProjectApiError` typed; caller does instanceof check).
  - `apps/cockpit/modules/projects/components/project-detail-view.tsx` (EDIT) — added imports (`useState`, `toast`, `Button`, `Dialog` + parts, `ProjectApiError`, `useRevokeApiKey`). `ApiKeysSection` + `ApiKeyRow` now take `projectId` prop. `ApiKeyRow` adds a "Revoke" button (`variant="outline" size="sm"`, only when `revokedAt === null`) + inlined `RevokeApiKeyDialog` component. Dialog uses `<Dialog>` primitive + Cancel/Revoke buttons; destructive variant on the confirm; Spinner inside button while pending. 422 ALREADY_REVOKED treated as benign race (toast.message + close + invalidate); 404 + 5xx + network surface as `toast.error`. Inlined rather than separate file because tightly coupled to row state — extract if a second consumer surfaces.
- **Pre-write attack notes vaulted in code comments:**
  - **422 ALREADY_REVOKED as benign race.** If the row is already revoked on the backend (concurrent revoke from another tab, or the operator double-clicked), the cockpit treats this as success-equivalent — invalidates the query + closes the dialog + neutral toast. No error UI. Matches the contract semantic (the desired end state is achieved).
  - **404 disambiguation cost: one extra tenant-scoped SELECT only on the error path.** Happy path stays single-query (`UPDATE … RETURNING`).
  - **Single-query approach for the UPDATE with the `revoked_at IS NULL` filter** — race-safe: two concurrent revokes from different operators race on the WHERE clause; one wins (returns the row), the other sees 0 rows + falls into the disambiguator + returns 422 with the (now non-null) revoked_at.
- **Contract divergences flagged (carried forward, NOT blocking):**
  - **`API_KEY_NOT_FOUND` + `ALREADY_REVOKED` UPPER_CASE codes vs vault contract lowercase `already_revoked`** — same casing-convention divergence as `keyPrefix` vs `prefix` on GET handler; consistent with `PROJECT_SLUG_EXISTS` in `createProjectHandler`. Amend the contract.
- **Slice 8.4 obligations REMAINING after this seat:**
  - **Smoke-script empirical run** (`/tmp/smoke-8-4-projects-admin.sh`) — still UNVERIFIED at runtime. K1 (RLS-INSERT-on-api-keys) is the load-bearing test; if K1 fails → SECURITY DEFINER migration owed. The new DELETE handler is NOT in the smoke script; extend before running, OR run as-is then extend.
  - **`GET /v1/orgs/me`** org-info backend endpoint (slice 8.3 cascade).
  - **Auth gate** `app/(app)/layout.tsx` (slice 8.3.2 prereq) — `/projects`, `/projects/new`, `/projects/{id}` are publicly reachable until then.
  - **`[[cockpit/first-run-journey]]` step 6 + step 5 terminology amendments** + **`[[cockpit/admin-list-endpoints-contract]]` `prefix` → `keyPrefix` + error-code casing amendments** owed in next /design pass.
  - **RSC migration for `<ProjectDetailView>` + `<ProjectsList>`** — first-run-journey step 4/11 specify RSC fetching via `React.cache(getSession)` + relative-path fetch through Vercel rewrite. Current Client Component pattern works behaviorally; RSC migration when auth gate lands.
  - **API-key issuance UI on detail page** — detail page can REVOKE but cannot ISSUE additional keys. Owed when API-key management UI lands (deferred slice).
- **Next on resume — recommended sequence:**
  1. **/implementation slice 8.3.2 — auth gate** `app/(app)/layout.tsx` + `modules/auth/lib/session.ts` RSC session-read via `React.cache(fetch '/api/auth/get-session')` per `[[cockpit-stack-integration-research]]` F5.6.
  2. **/implementation slice 8.3 — GET /v1/orgs/me** backend endpoint (org-display chrome).
  3. **/implementation — smoke-script empirical run** (operator-driven). Extend script first with DELETE coverage; then run with `DATABASE_URL` + `DATABASE_MIGRATION_URL` + `BOKCHOY_API_KEY_HMAC_SECRET` env.
  4. **/design pass — three contract amendments queued** (`prefix`→`keyPrefix`, error-code casing, first-run-journey step 5/6 terminology) + RSC migration plan for `<ProjectDetailView>`/`<ProjectsList>`.

- **Mode (prior):** implementation → slice 8.4 NEXT-CUT (`GET /v1/projects/{projectId}` + project detail page) LANDED 2026-05-13. Contracts satisfied: (A1) nested-`apiKeys[]`-on-project-detail per `[[cockpit/admin-list-endpoints-contract]]` + step 11 (T2) verify-key polling per `[[cockpit/first-run-journey]]`. 3 verification gates green: `bun run typecheck` ✓ (6 packages), cockpit `bun run lint` ✓ (31 files; +3 net new), cockpit `bun run dev` ✓ (Ready in 1154ms on port 3001, Cache Components enabled, no warnings, GET /projects/new 200).
- **Files landed this seat (4 new + 2 edits):**
  - `apps/backend/src/projects/index.ts` (EDIT) — added `getProjectHandler` returning `{...project, apiKeys: [...]}` bare-data per (R2) + (A1). OTel span `projects.get` with `bokchoy.api_key_count` attr. Project SELECT on plain `db` (no RLS); api_keys SELECT wrapped in `withTenant(db, projectId)` per FORCE-RLS policy in `packages/db/src/schema/api-keys.ts`. Field `keyPrefix` (matches Drizzle schema + matches existing `createApiKeyHandler` return shape; contract vault says `prefix` — amendment owed). Module header comment updated to "Four endpoints". Mounted on `app.get('/v1/projects/:projectId', ...)` with `adminGate({ resource: 'project', actions: ['read'], projectIdParam: 'projectId' })`. No `sValidator('param')` — adminGate Step 3 already validates UUID + tenancy.
  - `apps/cockpit/modules/projects/types.ts` (EDIT) — added `ApiKey` (full api_keys row shape with `lastUsedAt: string | null` + `revokedAt: string | null`; `keyPrefix` matches schema) and `ProjectDetail extends Project & { apiKeys: ApiKey[] }` types.
  - `apps/cockpit/modules/projects/api/get-project.ts` (NEW) — fetch wrapper through Vercel rewrite (`credentials: 'include'`), reuses `ProjectApiError` from `create-project.ts`.
  - `apps/cockpit/modules/projects/hooks/use-project.ts` (NEW) — TanStack `useQuery` with `queryKey: ['project', projectId]`, `staleTime: 0`, `refetchInterval` function predicate: poll every 3s while no `apiKey.lastUsedAt !== null` is observed; returns `false` to stop polling once verified. The 5-min hard timeout from the contract's Mitigations section is DEFERRED (no UI for "Still waiting?" hint yet — page-level useState can layer it on later without touching the query).
  - `apps/cockpit/app/(app)/projects/[id]/page.tsx` (NEW) — Server Component awaiting `Promise<{ id }>` per Next 16 dynamic-params shape; renders `<ProjectDetailView>`. No auth gate (slice 8.3.2 prereq; flagged in comment).
  - `apps/cockpit/modules/projects/components/project-detail-view.tsx` (NEW) — Client Component. Loading state (`<Spinner>`), error state (with "Back to projects" link), and three verify-key states inside a `<Card>`: "Key verified — last used N ago" (CheckCircle2 icon), "No API keys yet" (CircleAlert icon), or "Waiting for first SDK call…" (Spinner). Below that, `<ApiKeysSection>` renders one `<Card size="sm">` per key with KeyIcon + prefix + name + relative-time meta. Inline `formatRelative(iso)` helper for sub-minute fidelity ("just now" / "Ns ago" / "Nm ago" / "Nh ago" / "Nd ago") — Intl.RelativeTimeFormat rounded too aggressively for verify-loop semantics.
- **Pre-write attack notes vaulted in code comments:**
  - **TOCTOU race** on project DELETE between adminGate's SELECT (Step 3) + handler's SELECT — schema-impossible at slice 8.4 (`projects` FK from `api_keys.project_id` is ON DELETE CASCADE; `projects.organization_id` FK ON DELETE RESTRICT; no admin DELETE tooling exists). Handler throws on null row, which surfaces as 500 via `app.onError`. NOT a 404 — the contract's BC404 maps to AdminNotAMember in adminGate, not project-not-found in handler. Anti-improvisation: trust the gate.
  - **Polling stops on verified-OR-revoked-then-used key.** `apiKeys.some(k => k.lastUsedAt !== null)` includes revoked keys — if a key was used then revoked, polling stops (the verification already happened). VerifyKeyPanel's `find()` predicate adds `revokedAt === null` so the "Key verified" message points at a still-active key only.
  - **Verify-key predicate vs VerifyKeyPanel display predicate are intentionally different** — the polling-stop predicate (in `use-project.ts`) is "anything once used"; the display predicate (in `project-detail-view.tsx` VerifyKeyPanel) is "active + once used". Single-pass code review concern flagged in vault state.
- **Contract divergences flagged (owed amendments to vault, NOT blocking):**
  - **`keyPrefix` vs `prefix`** field name. Vault `[[cockpit/admin-list-endpoints-contract]]` says `prefix` in the GET /v1/projects/{id} response shape; landed code ships `keyPrefix` matching Drizzle schema + matching `createApiKeyHandler`'s pre-existing return shape. Amend the contract to say `keyPrefix` (or amend the codebase to map at the wire boundary — but the cost-benefit favors amending the contract since two cockpit consumers already depend on `keyPrefix`).
  - **5-minute polling timeout from contract Mitigations section** not implemented. Per anti-improvisation: it's in the Mitigations section, not the Decision section — not a strict contract. UI hint can land in a follow-on without backend or hook change.
  - **`step 11` verify-key UI verbatim text** says "Last used [N seconds] ago"; landed UI says "Last used Ns ago" (compact). Semantic match; cosmetic difference.
- **Slice 8.4 obligations REMAINING after this seat:**
  - **Smoke-script empirical run** (`/tmp/smoke-8-4-projects-admin.sh`) — still UNVERIFIED at runtime. K1 in particular tests RLS-INSERT-on-api-keys; if K1 fails → SECURITY DEFINER migration owed before slice ships.
  - **`DELETE /v1/projects/{id}/api-keys/{keyId}`** revoke handler + cockpit revoke action wired into the detail page's api-keys list.
  - **`GET /v1/orgs/me`** org-info backend endpoint (slice 8.3 cascade).
  - **Auth gate** `app/(app)/layout.tsx` (slice 8.3.2 prereq) — `/projects`, `/projects/new`, `/projects/{id}` are publicly reachable until then.
  - **`[[cockpit/first-run-journey]]` step 6 + step 5 terminology amendments** + **`[[cockpit/admin-list-endpoints-contract]]` `prefix` → `keyPrefix` amendment** owed in next /design pass.
  - **RSC migration for `<ProjectDetailView>`** + `<ProjectsList>` — first-run-journey step 4/11 specify RSC fetching via `React.cache(getSession)` + relative-path fetch. Current Client Component pattern works behaviorally; RSC migration when auth gate lands.
  - **API-key issuance UI on detail page** — current page READS keys but has no "Issue another key" button. Owed when API-key management UI lands (deferred slice).
- **Next on resume — recommended sequence:**
  1. **/implementation slice 8.4 — DELETE revoke handler** + cockpit revoke action. ~50 LOC backend + ~30 LOC cockpit; smallest remaining contract.
  2. **/implementation slice 8.4 — smoke-script empirical run.** Operator-driven with `DATABASE_URL` + `DATABASE_MIGRATION_URL` + `BOKCHOY_API_KEY_HMAC_SECRET` env. K1 RLS-INSERT-on-api-keys is the load-bearing test; if it fails, SECURITY DEFINER migration owed.
  3. **/implementation slice 8.3.2 — auth gate** `app/(app)/layout.tsx` + `modules/auth/lib/session.ts` RSC session-read via `React.cache(fetch '/api/auth/get-session')` per `[[cockpit-stack-integration-research]]` F5.6.
  4. **/design pass — three contract amendments + RSC migration plan.**

- **Mode (prior):** implementation → slice 8.4.0 (shadcn bootstrap) LANDED 2026-05-12. All 7 steps of `[[cockpit/shadcn-setup]]` *Cascade obligations* runbook completed. 3 verification gates green: `bun run typecheck` ✓, `bun run lint` ✓ (13 files), `bun run dev` ✓ (Ready in 2.6s, Cache Components enabled, no `turbopack.root` warning, no Turbopack rejection).
- **Init outcome (empirically verified):**
  - `apps/cockpit/components.json` created with `style: "radix-nova"` (combined base+preset identifier; NOT "new-york"), `rsc: true`, `tsx: true`, `tailwind.config: ""`, `baseColor: "neutral"`, `cssVariables: true`, full aliases map per `[[cockpit/shadcn-setup]]` *Decision*, `iconLibrary: "lucide"`, plus init-added `rtl: false`, `menuColor: "default"`, `menuAccent: "subtle"`, `registries: {}`.
  - `apps/cockpit/app/globals.css` rewritten with 30+ OKLCH tokens (`--background` through `--sidebar-ring`), `:root` + `.dark` paired blocks, `@import "tailwindcss"` + `@import "tw-animate-css"` + `@import "shadcn/tailwind.css"`, `@custom-variant dark (&:is(.dark *))`, full `@theme inline` block, `@layer base { * { ... } body { ... } html { @apply font-sans; } }`. Slice 8.3.1 comment block replaced with shadcn-managed-CSS doc per the new shape.
  - `apps/cockpit/lib/utils.ts` `cn()` helper retained (init rewrote stylistically; reverted import order + `:string` return type per cockpit Biome convention).
  - Deps added: `next-themes ^0.4.6`, `tw-animate-css ^1.4.0` (pre-init); `lucide-react ^1.14.0`, `class-variance-authority ^0.7.1`, `radix-ui ^1.4.3` (meta-package — NEW shape vs 2.x era; primitives import from `radix-ui`, not `@radix-ui/react-*`), `shadcn ^4.7.0` (production dep for `@import "shadcn/tailwind.css"` resolution). `tailwind-merge` BUMPED `^2.5.5 → ^3.6.0` (major version; `cn()` API unchanged).
  - `apps/cockpit/hooks/.gitkeep` created (components.json `aliases.hooks` requires dir exist).
  - `apps/cockpit/components/theme-provider.tsx` created from `[[cockpit/shadcn-setup-research]]` F5 verbatim canonical (NextThemesProvider + `d`-key hotkey + typing-target guards), in cockpit single-quote style.
  - `apps/cockpit/app/layout.tsx` wired: `suppressHydrationWarning` added to `<html>`, ThemeProvider wraps ReactQueryProviders (outermost), import via `@/components/theme-provider` alias.
  - NO `tailwind.config.ts` created (confirmed — Tailwind v4 has no JS config).
- **Three contract amendments LANDED in this seat** to `[[cockpit/shadcn-setup]]`:
  - **CLI invocation corrected** — `--preset nova` (bare name), NOT `--preset radix-nova` (the combined form is built internally by the CLI from `--base radix --preset nova`; not a valid `--preset` flag value). First-attempt failed with `Invalid preset: radix-nova. Available presets: nova, vega, maia, lyra, mira, luma, sera`. Vault entry's *Decision* + *Cascade obligations* invocation lines amended.
  - **components.json `style` value corrected** — init writes `"radix-nova"`, not `"new-york"` (which was the value I sourced from shadcn's `apps/v4/` site — that site predates the combined-identifier convention). Both are valid per `rawConfigSchema.style: z.string()`; cockpit ships `"radix-nova"`.
  - **Amendment block prepended at top of `[[cockpit/shadcn-setup]]`** with both corrections + 5 additional implementation-time findings (init's broken `--font-sans: var(--font-sans)` self-reference fix, tailwind-merge major bump, radix-ui meta-package shape, shadcn-as-runtime-dep, lint import-order auto-fix).
- **Implementation-time runtime bug caught + fixed:** init wrote `--font-sans: var(--font-sans);` (self-reference) and `--font-heading: var(--font-sans);` (cascade off self-reference) in `@theme inline`. For the cockpit which exposes `--font-geist-sans` / `--font-geist-mono` per slice 8.3.1's `next/font/google` setup, this would have made `font-sans` Tailwind utility resolve to nothing → primitives render in browser default. Rewrote to `var(--font-geist-sans)` / `var(--font-geist-mono)`. Documented in `[[cockpit/shadcn-setup]]` Amendment 2026-05-12.
- **Working tree status:** all post-init changes are STAGED in git (per pre-init `git add -A apps/cockpit/` step). No commits made (CLAUDE.md: no commits to main without authorization).
- **Slice 8.4.0 punch-list final status:**
  - Step 1 (pre-init deps): ✅ LANDED. `next-themes` + `tw-animate-css` added; `bun install` clean.
  - Step 2 (pre-init scaffold): ✅ LANDED. `hooks/.gitkeep` + `components/theme-provider.tsx` created.
  - Step 3 (wire ThemeProvider in layout.tsx): ✅ LANDED. suppressHydrationWarning + outermost wrap.
  - Step 4 (git stage): ✅ LANDED. `git add -A apps/cockpit/` pre-init.
  - Step 5 (init run): ✅ LANDED. `cd apps/cockpit && bunx shadcn@latest init --base radix --preset nova --yes` succeeded after first-attempt `radix-nova` correction.
  - Step 6 (post-init verification): ✅ LANDED. typecheck ✓ + lint ✓ + dev ✓ + components.json shape verified + globals.css 30+ tokens verified + no tailwind.config.ts.
  - Step 7 (cascade amendments confirmation): ✅ Already landed in /design seat (`[[cockpit/file-structure]]` cascade #3 obsolete + `[[cockpit/nextjs-scaffold-research]]` F4 superseded). Confirmed visible in vault.
- **shadcn primitive adds 2026-05-12 (slice 8.4.0 follow-on, 2 passes):** **10 primitives now in `apps/cockpit/components/ui/`:** `button`, `card`, `dialog`, `empty`, `field`, `input`, `label`, `separator`, `sonner`, `spinner`. Plus `<Toaster />` from `@/components/ui/sonner` mounted in `app/layout.tsx` inside `<ThemeProvider>` (after `<ReactQueryProviders>`) — needs ThemeProvider context for `useTheme()` to sync toast colors with class-based dark mode.
  - Pass 1 (button/input/label/dialog/field/separator): journey steps 1, 4, 5, 6, 7 — OAuth buttons + create-project form (RHF `<Controller>` + shadcn `<Field>` family) + visible-once modal.
  - Pass 2 (empty/card/sonner/spinner): journey steps 4 (empty-state UI), 8 (copy-to-clipboard toast feedback), 11 (polling loading state during T2 verify-key). User flagged that current shadcn catalog has primitives beyond 2.x training data — including `empty` as an explicit empty-state primitive (not a hand-rolled compose-from-card pattern).
  - Catalog enumerated via `bunx --bun shadcn@latest search @shadcn --limit 200` — ~57 `registry:ui` primitives at HEAD 2026-05-12. NEW relative to 2.x: `empty`, `button-group`, `input-group`, `item`, `kbd`, `native-select`, `spinner`, `direction`, `field` (form replacement).
  - Deferred (likely-soon, not strict slice-8.4 contract): `button-group` (OAuth button cluster step 1), `input-group` (slug-field with derived-from-name affix step 5). Add when those views ship.
  - `toast` primitive is DEPRECATED in shadcn — `sonner` is the canonical replacement (user shared upstream deprecation doc 2026-05-12).
  - **Initial `shadcn add button form input label dialog` resolved 4 files** (button/input/label/dialog) — `form` is a meta entry with 0 files. Replaced with explicit `add field` per [[feedback-shadcn-form-field-pattern]].
- **3 corrections to `[[cockpit/shadcn-setup]]` Amendment landed inline:**
  - **CLI invocation MUST include `--bun` flag** (`bunx --bun shadcn@latest …`); without it, bunx defaults to Node runtime. User directive — memory `feedback-bunx-bun-flag-for-shadcn.md` saved.
  - **`<Form>` family is gone; `<Field>` + react-hook-form `<Controller>` is the new pattern.** Per shadcn docs 2026-05-12: `field.tsx` exports `<Field>`, `<FieldLabel>`, `<FieldDescription>`, `<FieldError>`, `<FieldContent>`, `<FieldGroup>`, `<FieldSet>`, `<FieldLegend>`, `<FieldTitle>`. RHF integration is direct via `<Controller>` (no shadcn wrapper). Memory `feedback-shadcn-form-field-pattern.md` saved.
  - **Biome override for `components/ui/**`** — shadcn-vendored primitives use upstream conventions (import-style, quotes, `==`, semantic HTML choices) that conflict with cockpit Biome rules. Per `[[cockpit/file-structure]]` discipline (customize via module wrappers, NOT in primitive folder), treated `components/ui/**` as vendored-third-party: disabled `linter` + `formatter` + `assist` via `overrides` block in `apps/cockpit/biome.json`. Preserves clean shadcn update workflow against upstream registry. Lint + format remain active on cockpit's own code.
- **`[[cockpit/first-run-journey]]` step 5 amendment OWED** — references "RHF + Zod + shadcn `<Form>` + `<Input>`" (2.x terminology). Should be "RHF + Zod + shadcn `<Field>` + `<Input>` wrapped in react-hook-form `<Controller>`." Pure contract-terminology update; flag for next /design pass.
- **Slice 8.4 form-write deps INSTALLED 2026-05-12:**
  - `react-hook-form: ^7.62.0` → resolved 7.75.0 (above shadcn `apps/v4` production-cite floor of ^7.62.0)
  - `@hookform/resolvers: ^5.2.2` → resolved 5.2.2 (latest; v5 release coordinated with Zod v4 — supports both v3 and v4 zod schemas + `./standard-schema` resolver for vendor-neutral validation libs)
  - `zod: catalog:` → resolved 4.4.3 (inherited from monorepo workspace catalog at `package.json` root — aligns cockpit's Zod major with backend's, preventing schema-type drift across `@bokchoy/shared-types` boundary)
  - **Conflict resolved:** shadcn form-pattern docs use Zod v3; monorepo uses v4. Cockpit followed monorepo (workspace catalog wins over external production cite when the project-level constraint exists). `@hookform/resolvers@5.2.2` makes this clean — it supports both Zod versions transparently per its v5 release notes.
  - `bun install` ✓ (6 new packages including transitives); typecheck ✓; lint ✓.
- **Open threads:**
  - **`radix-ui` meta-package import pattern unverified.** First `shadcn add` calls in next slice will reveal whether components import as `import * as Dialog from 'radix-ui/Dialog'` or some other shape. Verify on first primitive add (probably `button` or `dialog`).
  - **`shadcn ^4.7.0` as runtime dep is unusual.** Required for `@import "shadcn/tailwind.css"` to resolve at build time. Worth flagging in `[[cockpit/shadcn-setup-research]]` open threads if it bites builds later.
  - **`d`-key dark-mode hotkey** is live by default per `theme-provider.tsx` canonical. Operator UX: pressing `d` (when not in a text input) toggles dark/light. If this collides with operator browser-extension shortcuts in practice, remove or gate on a config flag.
- **Slice 8.4 backend + cockpit work LANDED 2026-05-12.** 12 new files across `apps/backend/` + `apps/cockpit/modules/` + `apps/cockpit/app/` + 2 file edits:
  - **Backend (3 endpoints in 1 module):**
    - `apps/backend/src/projects/index.ts` (NEW) — `mountProjectsRoutes(app)`. Three handlers: `createProjectHandler` (POST /v1/projects with adminGate resource='project' action='create' + Zod body), `createApiKeyHandler` (POST /v1/projects/:projectId/api-keys with adminGate resource='apiKey' action='create' projectIdParam='projectId' + idempotencyMiddleware + 16-byte hex + HMAC-SHA256 via BOKCHOY_API_KEY_HMAC_SECRET + withTenant for RLS-protected INSERT), `listProjectsHandler` (GET /v1/projects with adminGate read + SELECT WHERE organization_id = admin.org.id ORDER BY createdAt DESC, bare array per (R2)). OTel spans 'projects.create', 'projects.create_api_key', 'projects.list'. 409 PROJECT_SLUG_EXISTS on unique-violation.
    - `apps/backend/src/index.ts` (EDIT) — added `mountProjectsRoutes(app)` mount + import. Biome re-ordered other imports.
    - **(E2) split-POST resolved.** Cockpit makes 2 sequential POSTs per [[cockpit/admin-list-endpoints-contract]]; (E1) bundling rejected. `[[cockpit/first-run-journey]]` step 6 "auto-issues" language is now outdated — contract amendment OWED.
    - **Hono generic invariance fix:** ProjectsAppContext widened to `ApiKeyContext & IdempotencyContext & AdminContext` (matches `apps/backend/src/index.ts` AppContext) so `mountProjectsRoutes(app: Hono<AppContext>)` accepts the parent without subtype-coercion failure. Same pattern as `mountWalletRoutes`.
  - **Cockpit form + modal wire (5 new files + 1 edit):**
    - `apps/cockpit/modules/projects/types.ts` — `Project`, `ApiKeyRecord`, `CreateProjectResponse`, `CreateApiKeyResponse`, `ApiError` types matching backend handler shapes. Inline duplication for now; promote to `@bokchoy/shared-types` if drift bites.
    - `apps/cockpit/modules/projects/api/create-project.ts` — POST /v1/projects fetch wrapper through Vercel rewrite (`credentials: 'include'`). Throws `ProjectApiError` on non-2xx.
    - `apps/cockpit/modules/projects/api/create-api-key.ts` — POST /v1/projects/{id}/api-keys with `Idempotency-Key: cockpit-first-key-${projectId}` default (override via input.idempotencyKey).
    - `apps/cockpit/modules/projects/hooks/use-create-project-and-key.ts` — TanStack `useMutation` chaining the two POSTs. Returns `{ project, apiKey, apiKeyRecord }`.
    - `apps/cockpit/modules/projects/components/api-key-modal.tsx` — shadcn `<Dialog>` visible-once modal. Copy button with `navigator.clipboard.writeText` + sonner toast on success + manual-copy fallback for clipboard-blocked contexts. K1 visible-once-on-creation.
    - `apps/cockpit/modules/projects/components/create-project-form.tsx` (EDIT) — `onSubmit` rewired from console.log stub to `createMutation.mutate(values, { onSuccess: setIssuedApiKey, onError: toast.error })`. Modal mounts inside form; dismiss → `router.push('/projects')`.
  - **Cockpit list view (4 new files):**
    - `apps/cockpit/modules/projects/api/list-projects.ts` — GET /v1/projects bare-array fetch.
    - `apps/cockpit/modules/projects/hooks/use-projects.ts` — `useQuery` with `queryKey: ['projects']`, `staleTime: 30s`.
    - `apps/cockpit/modules/projects/components/projects-empty-state.tsx` — shadcn `<Empty>` composition + `<Link href="/projects/new">New project</Link>` CTA. Per first-run-journey step 4.
    - `apps/cockpit/modules/projects/components/projects-list.tsx` — Client Component. Loading state (`<Spinner>`), error state, empty state, or Card grid. Each Card links to `/projects/{id}` (page lands in slice 8.4 next-cut).
    - `apps/cockpit/app/(app)/projects/page.tsx` (NEW) — Server Component header + CTA + `<ProjectsList />`. NO auth gate (slice 8.3.2 prereq).
  - **Verification 2026-05-12:** monorepo `bun run typecheck` ✓ (6 packages all green via turbo); cockpit `bun run lint` ✓ (21 files; shadcn primitives still excluded via biome.json `components/ui/**` override); cockpit `bun run dev` ✓ (Ready in 2.1s, Cache Components enabled, no warnings). Backend `bun run typecheck` ✓. Backend `bun run lint` ✓ (1 error auto-fixed via biome --write: `import type` for ApiKeyContext; pre-existing 2 warnings + 1 info in `packages/wallet/scripts/smoke-wrappers.ts` are NOT slice work).
  - **Biome config fix:** monorepo lint surfaced a "nested root configuration" conflict — `apps/cockpit/biome.json` was declaring itself as root (alongside the monorepo's `/biome.json` root). Added `"root": false` to cockpit's biome.json + dropped its `vcs` block (root-only field per Biome 2.x). Biome now treats the cockpit config as nested-extending-parent.
  - **Slice 8.4 follow-on fixes LANDED 2026-05-12** (4 issues addressed pre-ship):
    - **Cosmetic:** dropped unused `Button` import + `void Button;` hack from `apps/cockpit/modules/projects/components/projects-list.tsx`.
    - **Port collision:** `apps/cockpit/package.json` `dev` script pinned to port 3001 (`next dev --turbopack -p 3001`); leaves backend's default 3000 unblocked for parallel `bun run dev`. `.env.example` note expanded with the rationale.
    - **Slice 8.4 integration smoke** at `/tmp/smoke-8-4-projects-admin.sh` — 11 tests + 2 DB-state verifications. Critical test K1 verifies the slice 8.1a script-author's warning about `withTenant` + RLS INSERT on api_keys ("INSERT from bokchoy_app would require a SECURITY DEFINER function"). Script is syntax-checked but UNVERIFIED at runtime — operator runs with `DATABASE_URL` + `DATABASE_MIGRATION_URL` + `BOKCHOY_API_KEY_HMAC_SECRET` env set. If K1 fails with an RLS policy violation, we owe a SECURITY DEFINER migration before slice 8.4 ships.
    - **Orphan-project UX bug fixed:** `useCreateProjectAndKey` now throws `ProjectCreatedButKeyFailedError` carrying the orphaned `projectId` + `project` row when step 2 fails after step 1 succeeded. Form catches the typed error and switches to a retry-key state (banner explaining the situation + "Retry key creation" button calling just step 2 via new `useRetryApiKey` hook + "Skip — I'll create a key later" escape hatch). Same `Idempotency-Key: cockpit-first-key-${projectId}` on retry so idempotencyMiddleware replays the cached response if step 2 actually committed but the cockpit lost the network response.
    - Verification 2026-05-12: monorepo typecheck ✓, cockpit lint ✓ (27 files; shadcn primitives still excluded), cockpit dev ✓ (Ready in 1625ms on port 3001, no warnings, Cache Components enabled).
  - **Slice 8.4 obligations still owed:**
    - **Smoke-script empirical run** (operator-driven; verifies the RLS-INSERT-on-api-keys + 10 other behaviors end-to-end). Until run, slice 8.4 is "compiles + structurally sound" not "verified-functional."
    - Project detail page `/projects/{id}` per first-run-journey step 11 (T2 verify-key polling view) — slice 8.4 next-cut. ProjectCard links 404 until then.
    - `GET /v1/projects/{id}` backend handler with nested `api_keys[]` per (A1) — also slice 8.4 next-cut.
    - `DELETE /v1/projects/{id}/api-keys/{keyId}` revoke handler — slice 8.4 next-cut (API-key management UI).
    - `GET /v1/orgs/me` handler for org-info display — slice 8.3 cascade, separate.
    - **`[[cockpit/first-run-journey]]` step 6 amendment OWED** — text says "Backend auto-issues first SDK API key" implying (E1) bundling; reality is (E2) split-POST with cockpit chaining. Amend in next /design pass.
    - **`[[cockpit/first-run-journey]]` step 5 terminology amendment OWED** — references shadcn `<Form>`; should be `<Field>` + RHF `<Controller>` (still owed from earlier).
    - **Auth gate `(app)/layout.tsx`** — slice 8.3.2 prereq. `/projects` + `/projects/new` are publicly reachable until then.
    - **RSC migration for `<ProjectsList>`** — first-run-journey step 4 specifies RSC fetching via `React.cache(getSession)` + relative-path fetch through Vercel rewrite. Current Client Component pattern works behaviorally; RSC migration when auth gate lands.

- **Slice 8.4 step 5 form scaffold LANDED 2026-05-12 (prior).** Three new files:
  - `apps/cockpit/modules/projects/lib/create-project-schema.ts` — Zod v4 schema (`name` 1-64 chars, `slug` 1-64 kebab-case) + `deriveSlug()` helper for name→slug auto-derivation. Per [[cockpit/first-run-journey]] step 5 constraints + [[cockpit/admin-list-endpoints-contract]] POST /v1/projects payload.
  - `apps/cockpit/modules/projects/components/create-project-form.tsx` — Client Component. RHF `useForm({ resolver: zodResolver(...), mode: 'onBlur' })` + two `<Controller>` blocks wrapping shadcn `<Field>` family per [[cockpit/shadcn-setup]] Amendment 2026-05-12 form-pattern memo. Auto-slug behavior: `useState(autoSlug)` flips to false on first user-driven slug edit, preserving manual edits across subsequent name typing. `onSubmit` is a STUB (console.log) — POST /v1/projects backend handler is a separate slice 8.4 cascade.
  - `apps/cockpit/app/(app)/projects/new/page.tsx` — Server Component rendering the form under route group `(app)/`. NO auth-gate enforcement yet (DEFERRED to slice 8.3.2 Better Auth client provider + lib/session.ts RSC session-read); route is technically public. Flag tracked.
  - Form file landed with 1 Biome warning auto-fixed (unnecessary parens around single-expression conditional in JSX); structure verbatim against shadcn Field-component docs the user shared 2026-05-12.
  - All three gates ✓: typecheck, lint (16 files; +3 new from 13), dev (Ready in 1991ms).
- **Slice 8.4 obligations still owed:**
  - **`POST /v1/projects` backend handler** (`apps/backend/src/projects/createProjectHandler.ts` per `[[cockpit/admin-list-endpoints-contract]]` cascade) — separate slice. Once landed, cockpit form's `onSubmit` rewires from console.log to a TanStack Query mutation with idempotency key + visible-once modal trigger on success.
  - **Visible-once API-key modal** (`<Dialog>` per first-run-journey step 7). Compose with the form's onSuccess transition.
  - **Auth gate** `(app)/layout.tsx` enforcing session-read via `lib/session.ts` (slice 8.3.2 prereq).
  - **`[[cockpit/first-run-journey]]` step 5 terminology amendment** owed in next /design pass — "shadcn `<Form>` + `<Input>`" → "shadcn `<Field>` + `<Input>` wrapped in RHF `<Controller>`".
- **Next on resume — recommended sequence:**
  1. **/implementation slice 8.4 — backend handler.** `apps/backend/src/projects/` new module with `createProjectHandler` (+ `getOrgMeHandler`, `listProjectsHandler`, `getProjectHandler`, `createApiKeyHandler`, `revokeApiKeyHandler` per the contract). Behind `adminGate`. Drizzle schema verification first (`projects.name`, `(organization_id, slug)` unique, `api_keys.name`, `api_keys.revoked_at`).
  2. **/implementation slice 8.4 — wire form to backend.** Replace `onSubmit` console.log with TanStack Query mutation; surface BC4xx via sonner toast; trigger visible-once `<Dialog>` modal on success per first-run-journey step 7.
  3. **/implementation slice 8.4 — projects list view.** `app/(app)/projects/page.tsx` composing `<ProjectsList />` from `modules/projects/components/projects-list.tsx`; uses `<Empty>` for the empty state per step 4, `<Card>` per project row.
  4. **/implementation slice 8.3.2 — auth gate.** `app/(app)/layout.tsx` + `modules/auth/lib/session.ts` RSC session-read via `React.cache(fetch '/api/auth/get-session')` per `[[cockpit-stack-integration-research]]` F5.6.

- **Mode (prior):** design → `[[cockpit/shadcn-setup]]` LANDED 2026-05-12. All three shadcn picks resolved + one forced move ratified + two cascade amendments LANDED in same seat. Slice 8.4 bootstrap UNBLOCKED for /implementation.
- **Picks resolved (user defenses on file, both engaging the specific failure mode named for the pick):**
  - **`--base radix`** — user: *"ecosystem maturity matters more than upstream signal."* Inverts lock-in-tax-on-deprecation attack (Radix exists independently of shadcn-as-CLI). Production-cited × many.
  - **`--preset radix-nova`** — user: *"it's an admin tool plus Lucide has more icons and more mature."* Admin-tool register defangs visual-undifferentiation attack; Lucide ~1500 catalog defeats Phosphor coverage-gap concern; Geist matches slice 8.3.1 verbatim (zero font swap).
  - **Dark mode `next-themes` class-based** — forced move, not a fork (shadcn `@custom-variant dark (&:is(.dark *))` requires class selector; media query doesn't activate it). Vaulted as ratification.
  - **(M-LOCAL) cockpit-local primitives** — re-stated; user directive 2026-05-12 + Issue #6878 (Tailwind v4 monorepo content-scanning bug) precedent.
- **Vault writes this seat:**
  - `.bocek/vault/cockpit/shadcn-setup.md` (NEW) — full decision entry with 7 rejected alternatives, 4 failure modes + mitigations, 7-step implementation runbook, 6 revisit-when triggers.
  - `.bocek/vault/cockpit/file-structure.md` (AMENDED) — cascade #3 marked OBSOLETE (Tailwind v4 has no JS config); open thread "shadcn `components.json` config path" marked RESOLVED with full aliases map. Amendment block 2026-05-12 added pointing at `[[cockpit/shadcn-setup]]`.
  - `.bocek/vault/cockpit/nextjs-scaffold-research.md` (AMENDED) — F4 SUPERSEDED for shadcn-adopting cockpit. Inline amendment block points at `[[cockpit/shadcn-setup-research]]` F6 / C1. Per *Contradiction protocol*: shadcn-v4 canonical wins over create-next-app canonical for consumers adopting shadcn.
  - `.bocek/vault/index.md` (AMENDED) — new `[[cockpit/shadcn-setup]]` entry added at top of cockpit section.
- **Pattern observation (3 picks, 2026-05-12):** user trends conservative — (M-LOCAL) on single-app workspace, (B-RADIX) on ecosystem maturity, (P-NOVA) on default-shadcn / minimum-friction. Pattern matches MVP-solo-dev context (defer novelty for shippability); not pathological deferral. Re-evaluate at slice 8.5+ if pattern persists when ambition-bearing decisions arise (catalog editor UX, transaction inspector data density).
- **Implementation runbook for slice 8.4 shadcn bootstrap (vaulted in `[[cockpit/shadcn-setup]]` *Cascade obligations*):**
  1. Add deps: `next-themes ^0.4.6`, `tw-animate-css`, optional `prettier-plugin-tailwindcss ^0.7.2`.
  2. Pre-init scaffold: `apps/cockpit/hooks/.gitkeep`; `apps/cockpit/components/theme-provider.tsx` (verbatim from `[[cockpit/shadcn-setup-research]]` F5).
  3. Wire `<ThemeProvider><ReactQueryProviders>{children}</ReactQueryProviders></ThemeProvider>` in `app/layout.tsx`; add `suppressHydrationWarning` to `<html>`.
  4. Stage git: `git add -A apps/cockpit/` (mandatory rollback safety).
  5. Run: `cd apps/cockpit && bunx shadcn@latest init --base radix --preset radix-nova --yes`.
  6. Post-init verification: components.json shape diff, globals.css 30+ OKLCH tokens, NO `tailwind.config.ts`, `bun run dev` ✓ + typecheck ✓ + lint ✓.
  7. Confirm cascade amendments to `[[cockpit/file-structure]]` + `[[cockpit/nextjs-scaffold-research]]` are visible in vault (already done in this /design seat — implementation just verifies no-regression).
- **Next on resume — recommended sequence:**
  1. **/implementation slice 8.4.0 (shadcn bootstrap)** per the 7-step runbook above. Quote `[[cockpit/shadcn-setup]]` *Decision* + *Cascade obligations* as the contract. Verify against the *Failure mode* + *Mitigations* sections.
  2. **/implementation slice 8.4** — project creation flow + API-key issuance per `[[cockpit/admin-list-endpoints-contract]]` + `[[cockpit/first-run-journey]]` step 5-7. First `shadcn add` calls land here (button, input, form, dialog, label).

- **Mode (prior):** research → `[[cockpit/shadcn-setup-research]]` LANDED 2026-05-12. Triangulated current canonical shadcn CLI setup for Next.js 16 + Tailwind v4 + RSC against `shadcn-ui/ui` HEAD (pushed 2026-05-12T08:27:02Z) + `ui.shadcn.com/docs/{installation/next,tailwind-v4}` + contradiction probe (4 open GitHub issues on monorepo+Tailwind v4 friction). 12 findings (F1–F12), 3 conflicts (C1–C3), 13 operational obligations.
- **Triggered by user halt 2026-05-12** when /implementation seat was about to scaffold from 2.x-era assumptions. User: *"we are not going to put in package only cockpit will use it"* — (M-LOCAL) `apps/cockpit/components/ui/` ratified on-the-spot; (M-SHARED) `packages/ui/` workspace extraction REJECTED. Memory `feedback-fast-moving-oss-research-first.md` saved before research started.
- **Load-bearing findings (verbatim production-cite anchored):**
  - **F1:** CLI npm package renamed `shadcn-ui` → **`shadcn`**; current version **`4.7.0`**. 2.x invocation is wrong.
  - **F2:** init prompt flow restructured — NO standalone style/baseColor/iconLibrary prompts; new flow asks `base` (radix vs base-ui) then **named preset** bundling everything.
  - **F4:** components.json schema — `tailwind.config: ""` (Tailwind v4 has NO JS config), `rsc: true` (App Router), `cssVariables: true`, `aliases.{components,utils,ui,lib,hooks}`.
  - **F5:** dark-mode = `next-themes` class-based (`.dark` on `<html>` via `attribute="class"`, `defaultTheme="system"`). Cockpit's slice 8.3.1 `@media (prefers-color-scheme: dark)` is the OLD canonical.
  - **F6:** globals.css canonical = `@import "tailwindcss"; @import "tw-animate-css"; @import "shadcn/tailwind.css"; @custom-variant dark (&:is(.dark *));` + 30+ OKLCH tokens + base layer. Slice 8.3.1's 4-token `@theme` block + hex colors is structurally out of date.
  - **F7:** `shadcn/tailwind.css` is exported infrastructure — `@custom-variant data-{open,closed,...}` wires Radix/Base data-state attributes. NOT optional.
  - **F8:** Issue #6878 OPEN — monorepo `packages/ui/` + Tailwind v4 has content-scanner bug; primitives render unstyled. (M-LOCAL) sidesteps entirely (user directive validated by production-derived defense).
  - **F9:** `shadcn init` must run from `apps/cockpit/` (CLI exits at monorepo root). PostCSS deps already in cockpit per slice 8.3.1 — friction neutralized.
  - **F10:** NEW `(B-RADIX) vs (B-BASE)` design fork — `--base` enum is `radix | base`. Base UI (by Floating UI team) is shadcn's newer-recommended path. Needs Base UI maturity research before /design picks.
- **3 amendments owed to other vault entries:**
  - **`[[cockpit/file-structure]]` cascade #3 OBSOLETE** — `apps/cockpit/tailwind.config.ts` should be dropped; Tailwind v4 has no JS config. Amend to point at `[[cockpit/shadcn-setup-research]]` F6 for the new canonical globals.css shape.
  - **`[[cockpit/nextjs-scaffold-research]]` F4 superseded** — for cockpit (which adopts shadcn), shadcn-v4 globals.css canonical wins over create-next-app's. Add amendment note pointing at `[[cockpit/shadcn-setup-research]]` F6 / C1.
  - **New `[[cockpit/shadcn-setup]]` /design entry** owed for (B-RADIX) vs (B-BASE) pick + preset pick (Nova/Vega/Maia/Lyra/Mira/Luma/Sera). Or fold into `[[cockpit/cockpit-shape]]` I1.
- **Files written this session:**
  - `.bocek/vault/cockpit/shadcn-setup-research.md` (NEW, ~7000 words, 12 findings, 3 conflicts, 13 operational obligations, full preset taxonomy + components.json schema + verbatim globals.css canonical + reproducibility commands).
  - `.bocek/vault/index.md` — new cockpit entry added at top of cockpit section.
- **Next on resume — recommended sequence:**
  1. **(Optional pre-design /research)** — Base UI maturity assessment if /design seriously considers (B-BASE). Primitive coverage, license, maintenance cadence, 2025-2026 production cites. ~30min focused pass. Skip if defaulting to (B-RADIX) as provisional pick.
  2. **/design pass** — resolve 3 picks: (a) `base: radix | base`, (b) preset: nova (recommended, matches slice 8.3.1 Geist pick) / vega / maia / lyra / mira / luma / sera, (c) confirm dark-mode mechanism switch from media-query → next-themes class-based. Vault as `[[cockpit/shadcn-setup]]` or sub-decision of `[[cockpit/cockpit-shape]]`. Also amend `[[cockpit/file-structure]]` cascade #3 + `[[cockpit/nextjs-scaffold-research]]` F4 in same pass.
  3. **/implementation slice 8.4.0 (shadcn bootstrap)** — pre-init prep (add `next-themes` + `tw-animate-css` deps; create `apps/cockpit/hooks/.gitkeep`; create `apps/cockpit/components/theme-provider.tsx` from F5 canonical; wire into layout with `suppressHydrationWarning`; replace `app/globals.css` with F6 canonical or minimum `@import "tailwindcss"`). Then `cd apps/cockpit && bunx shadcn@latest init --base <pick> --preset <pick> --yes`. Verify `bun run dev` + typecheck + lint.
  4. **/implementation slice 8.4** — project creation flow + API-key issuance per `[[cockpit/admin-list-endpoints-contract]]` + `[[cockpit/first-run-journey]]` step 5-7.

- **Mode (prior):** implementation → slice 8.3.x cleanup item #10 (`turbopack.root` verify) RESOLVED 2026-05-12. **F6 of `[[cockpit/nextjs-scaffold-research]]` ("redundant pin" claim) FALSIFIED** — Next.js 16.2.6's lockfile auto-detect walks UP from cwd and selected a stray `/home/mvula/package-lock.json` (outside the monorepo, in operator's `$HOME`) as the workspace root, demoting `apps/../bun.lock` to an "additional lockfile". The pin is load-bearing, not redundant. F6 is walked back; pin stays.
- **Latent `next.config.ts` bug ALSO surfaced + fixed in same slice.** Prior `next.config.ts` shape used runtime imports (`import { dirname, resolve } from 'node:path'` + `import { fileURLToPath } from 'node:url'`) to compute `__dirname` for the pin. typecheck + lint passed through slices 8.3.0 + 8.3.1 but **`next dev` CRASHED** with `ReferenceError: exports is not defined in ES module scope` — Next.js's config loader emits CJS-style `exports.X = ...` to `next.config.compiled.js`, which collides with `apps/cockpit/package.json` `"type": "module"`. Bug was invisible until tonight because slice 8.3 verified typecheck + lint only; `next dev` was never run.
- **Fix:** `next.config.ts` rewritten to drop all runtime `node:*` imports. Replacement `turbopack.root` uses `new URL('../..', import.meta.url).pathname` — `new URL` constructor + `import.meta.url` are language built-ins (no import needed), and the URL parser normalizes `..` segments. `import.meta.dirname` alternative was attempted first; config loaded fine but produced an unnormalized path (`/.../apps/cockpit/../..`) that Turbopack rejected with "files outside of the project directory will not be compiled" — `new URL()` was needed specifically for path normalization.
- **Verification 2026-05-12:**
  - `bun run dev` (apps/cockpit) ✓ — Ready in 1206ms; NO "inferred workspace root" warning; NO Turbopack rejection; Cache Components enabled.
  - `bun run typecheck` (`tsc --noEmit`) ✓ — no errors.
  - `bun run lint` (`biome check`) ✓ — 11 files, no errors.
- **Files changed this slice:**
  - `apps/cockpit/next.config.ts` — dropped `node:path` + `node:url` imports + `__dirname` computation; replaced `resolve(__dirname, '../..')` with `new URL('../..', import.meta.url).pathname`; comment block expanded to document (a) F6 walkback rationale (stray `~/package-lock.json` empirically broke auto-detect), (b) CJS/ESM loader-collision rationale for avoiding runtime imports in ESM-mode Next.js config, (c) why `new URL` was chosen over `import.meta.dirname` (path normalization required by Turbopack's outside-project check).
- **`[[cockpit/nextjs-scaffold-research]]` amendments LANDED 2026-05-12 (2 items):**
  1. **F6 walkback inline** — "redundant pin" claim falsified per 2026-05-12 empirical run; pin is load-bearing on any operator machine with stray lockfiles in `$HOME` or above the repo. Amendment block added at F6 with verbatim warning output + `$HOME`-stray-lockfile failure-mode analysis.
  2. **NEW F9 LOAD-BEARING finding** — Next.js 16's `next.config.ts` config loader emits CJS to `.compiled.js`, incompatible with `"type": "module"` packages when the config has any runtime `import`. F9 documents trigger + workaround (language built-ins only: `import type`, `new URL`, `import.meta.url`, `import.meta.dirname`, globals like `process`, `URL`) with pattern table + verified-pattern code example. Severity flagged LOAD-BEARING because typecheck + lint stay green while dev/build silently break.
  3. **Operational implication #10 RESOLVED stamp + new #12 added** — #12: future `next.config.ts` edits MUST include `bun run dev` in the verification gate, not just typecheck + lint.
  4. **Vault index entry updated** with amendment summary so future seats see walkback + F9 at a glance without opening the entry.
- **Slice 8.3.x cleanup punch-list final status:**
  - Items #1-#6, #8, #9, #11: ✅ LANDED 2026-05-12 (slice 8.3.1).
  - **#7 favicon** — STILL DEFERRED. Needs BokChoy branding /design pick; not actionable from /implementation.
  - **#10 turbopack.root verify** — ✅ RESOLVED 2026-05-12 (this slice). F6 walkback owed + latent loader bug fixed in same pass.
- **Open threads still owed:**
  - Two amendments to `[[cockpit/nextjs-scaffold-research]]` queued (F6 walkback + new ESM-mode-import finding). Pure documentation; not blocking slice 8.4.
  - **Slice 8.4 (project creation + API-key issuance)** is now unblocked AND verified-runnable in dev. Cascade obligations are vaulted at `[[cockpit/admin-list-endpoints-contract]]` + `[[cockpit/first-run-journey]]` step 5-7.
  - **shadcn CLI bootstrap** still queued as slice 8.4 prerequisite — `globals.css` `@theme` registrations are in place from slice 8.3.1.
- **Next on resume — recommended sequence:**
  1. ✅ Research-entry amendments LANDED 2026-05-12 (F6 walkback + new F9 + operational #10/#12 + index amendment).
  2. **/research the current shadcn CLI setup** for Next.js 16 + Tailwind v4 + RSC BEFORE bootstrapping. User flagged 2026-05-12 in /implementation seat that the shadcn setup I was about to scaffold (style/baseColor/iconLibrary/components.json + class-based dark mode prompts) is the **old** setup — current shadcn shape differs and must be verified from primary sources before /implementation can act. Recommend scope: current shadcn CLI version + init flow + Tailwind v4 integration + Next.js 16 compatibility + production cites (Vercel/shadcn-own-site/canary Next.js demos). See `feedback-fast-moving-oss-research-first.md` user memory for the meta-pattern.
  3. (/design after /research) — pick the shadcn-decision forks surfaced by the research (style variant, base palette, icon library, dark-mode mechanism, deps, components.json shape) per `[[cockpit/file-structure]]` cascade #4. May fold into the existing `[[cockpit/cockpit-shape]]` or land as new `[[cockpit/shadcn-setup]]`.
  4. (Implementation) shadcn CLI bootstrap to `apps/cockpit/components/ui/` per slice 8.4 prerequisite — quote contracts from research + design.
  5. (Implementation) slice 8.4 — project creation flow + API-key issuance.

- **Mode (prior):** implementation → slice 8.3.1 scaffold-polish 8-of-11 punch-list items LANDED 2026-05-12. `bun run typecheck` ✓ + `bun run lint` (Biome) ✓ both green. Items #7 (favicon — needs BokChoy branding /design pick) + #10 (turbopack.root verify — needs dev-server observation) deferred. Item #11 was passive (do-not-regress confirmations).
- **User-ratified directives 2026-05-12 (in /implementation seat, on-the-spot):**
  - **#1 Linter:** Biome (diverges from research recommendation of ESLint — research said "recommend ESLint default until pressure to switch"; user accepted Biome's speed + single-tool simplicity).
  - **#6 JSX:** `react-jsx` (matched research recommendation).
  - **#9 Font:** Geist now (matched research recommendation + canonical create-next-app).
- **Files changed in slice 8.3.1 polish:**
  - `apps/cockpit/package.json` — added `@biomejs/biome@2.4.2` devDep + `lint: "biome check"` + `format: "biome format --write"` scripts.
  - `apps/cockpit/biome.json` (NEW) — Source 1 verbatim + **3 monorepo-forced deviations** documented inline: (a) `vcs.useIgnoreFile: false` (canonical assumes biome.json alongside .gitignore; BokChoy's .gitignore is at monorepo root + Biome 2.4.2's `vcs.root` field did not resolve via relative path — pragmatic drop; `files.includes` already excludes node_modules/.next/dist/build); (b) `javascript.formatter.quoteStyle: "single"` (matches existing BokChoy single-quote style in `apps/backend/src/`; canonical defaults to double); (c) `files.includes` adds `!next-env.d.ts` (auto-generated, gitignored, must not lint).
  - `apps/cockpit/tsconfig.json` — `jsx: "preserve"` → `"react-jsx"`; dropped `types: ["node"]` restriction; added `**/*.mts` + `.next/dev/types/**/*.ts` to `include`.
  - `apps/cockpit/app/layout.tsx` — added `next/font/google` Geist + Geist_Mono with `--font-geist-sans`/`--font-geist-mono` CSS variables on `<html>` + canonical `min-h-full flex flex-col` body class. Preserved `ReactQueryProviders` wrap.
  - `apps/cockpit/app/globals.css` — Source 1 verbatim expansion: `:root` CSS vars + `@theme inline` block (color tokens + font tokens) + dark-mode `prefers-color-scheme` media query + body default fallback.
  - `apps/cockpit/.env.example` (NEW) — documents `BOKCHOY_BACKEND_URL` per `[[cockpit/auth-surface-mount]]` (V3); no OAuth env (cockpit has no Better Auth instance per `[[cockpit-stack-integration-research]]` F4).
  - `apps/cockpit/next.config.ts` — Biome organize-imports auto-fix swapped `node:url` / `node:path` order (cosmetic only).
  - `apps/cockpit/app/page.tsx` + `apps/cockpit/lib/react-query-providers.tsx` — Biome formatter long-line re-wrap (cosmetic only).
  - `.gitignore` (root) — added `**/next-env.d.ts` per `[[cockpit/nextjs-scaffold-research]]` F3 + Next.js docs recommendation.
- **Cockpit scaffold verification 2026-05-12:**
  - `bun install` ✓ — 6 packages installed (Biome + transitives), lockfile saved.
  - `bun run typecheck` (`tsc --noEmit`) ✓ — no errors.
  - `bun run lint` (`biome check`) ✓ — 11 files checked, no errors.
- **Remaining punch-list items DEFERRED:**
  - **#7 favicon** — needs BokChoy-branded `app/favicon.ico` (Next.js 16 file convention, NOT `public/favicon.ico` — research entry was wrong on path). Shipping canonical create-next-app's Next.js-triangle favicon as BokChoy's identity is worse than the harmless 404. Carry to slice 8.3.3 or whenever branding lands.
  - **#10 turbopack.root verify** — current pin `resolve(__dirname, '../..')` is defensive; per Next.js 16 docs Turbopack auto-detects workspace root via `bun.lock` (present at `/home/mvula/audhd/bokchoy/bun.lock`). Verify by removing the pin + running `next dev` + observing whether Next.js logs the correct root. Cleanup pass.
  - **#11 do-not-regress confirmations** — passive: `cacheComponents: true`, `reactCompiler: true`, `babel-plugin-react-compiler@^1.0.0`, Tailwind v4 PostCSS shape, `lib/query-client.ts` `React.cache` wrap all UNCHANGED. ✓
- **Open threads still owed:**
  - The 4 LOAD-BEARING claims from `[[cockpit/nextjs-scaffold-research]]` (F1-F4) are now RESOLVED in code. Research entry remains the audit-trail for why these changes were made.
  - **Biome-vs-ESLint** is now SETTLED for cockpit (Biome picked). Carry to `[[frontend-stack]]` amendment if cockpit-Biome choice should extend to backend (currently no linter there per inspection — `apps/backend/` has no `eslint.config.mjs` either; separate decision).
  - **`turbopack.root` redundancy verification** — single dev-server run away.
  - **shadcn CLI bootstrap (slice 8.4)** — now UNBLOCKED. `globals.css` has the `@theme` registrations shadcn primitives need.
- **`[[cockpit/nextjs-scaffold-research]]` cascade obligations queued (ordered by severity for slice 8.3.1):**
- **`[[cockpit/nextjs-scaffold-research]]` cascade obligations queued (ordered by severity for slice 8.3.1):**
  1. Add ESLint to cockpit — `eslint@^9` + `eslint-config-next` + `apps/cockpit/eslint.config.mjs` (copy Source 1 verbatim) + `lint: "eslint"` script + CI wiring. (BLOCKER — F1)
  2. Drop `types: ["node"]` from `apps/cockpit/tsconfig.json` or expand to `["node", "react", "react-dom"]`. (F2)
  3. Add `.next/dev/types/**/*.ts` + `**/*.mts` to `apps/cockpit/tsconfig.json` `include`. (F2)
  4. Expand `apps/cockpit/app/globals.css` to canonical shape — CSS variables + `@theme inline` block + dark-mode media query + body defaults. **Required before slice 8.4 shadcn CLI bootstrap.** (F4)
  5. Add `**/next-env.d.ts` to monorepo root `.gitignore` (currently MISSING, verified 2026-05-12). (F3)
  6. Switch `jsx: "preserve"` → `"react-jsx"` in `apps/cockpit/tsconfig.json`. (F2)
  7. Add `apps/cockpit/public/favicon.ico` placeholder. (F7)
  8. Add `apps/cockpit/.env.example` documenting `BOKCHOY_BACKEND_URL` + future OAuth env. (F7)
  9. Pick or document deferral for `next/font/google` Geist setup. (F5)
  10. Verify `turbopack.root` necessity — try removing, see if `bun.lock` auto-detect works. (F6)
  11. Confirmations (do NOT regress): `cacheComponents: true` + `reactCompiler: true` + `babel-plugin-react-compiler@^1.0.0` + Tailwind v4 PostCSS + `lib/query-client.ts` React.cache. (F8)
- **Open threads from `[[cockpit/nextjs-scaffold-research]]`:**
  - ESLint vs Biome — both canary-template first-class; recommend ESLint default. /design pick owed.
  - `jsx: "preserve"` vs `"react-jsx"` empirical perf delta in Next.js 16 SWC pipeline — both work end-to-end; canonical is `react-jsx`.
  - `next-env.d.ts` auto-regen path (`types/` vs `dev/types/`) — auto-managed, gitignored, not load-bearing.
  - Bun + Next.js 16 + Tailwind v4 PostCSS plugin discovery gotchas — not surveyed; investigate if `bun run dev` fails.
  - Cockpit testing strategy (Playwright + Vitest per `[[frontend-stack]]`) — still deferred; not in this scope.
- **Mode (prior):** design → 5/5 cockpit /design entries LANDED + 2 mechanical amendments LANDED 2026-05-11. Cockpit /design pass COMPLETE.
- **`[[cockpit/file-structure]]` LANDED 2026-05-11.** (P-CAL) Cal.com-style vertical-slice modules ratified. `apps/cockpit/modules/{feature}/{components,api,hooks,lib,types}/` + `app/` route groups + `components/{ui,layouts}/` shared + `lib/` cross-feature utilities. 4 rejected alternatives with named winning conditions: (P-LAYER) Documenso/Trigger.dev layer-based (cite-count-not-cite-weight); (P-DUB) Dub.co horizontal split (internal-consistency-loss); (P-VIEW) `*-view.tsx` page-view abstraction (no testability/reuse demand); workspace `packages/cockpit-features/` extraction (single-app over-engineering). 3 failure modes + 6 revisit-when triggers + 10 cascade obligations for slice 8.3 + 8.4 implementation.
- **`[[backend-stack]]` §7 line 116 amendment LANDED 2026-05-11.** Cockpit web login changed from "email/password + magic-link (or OAuth on demand)" to "OAuth Google/GitHub primary + email/password fallback; magic-link deferred." Inline amendment-note added pointing at `[[cockpit-stack-integration-research]]` F7 + `[[cockpit-shape]]` design pick (iii).
- **`[[backend-stack-research]]` Source 8 cite-class amendment LANDED 2026-05-11.** Erratum block prepended noting Cal.com is on `next-auth: 4.24.13` (Auth.js v4 security-patch-only mode), NOT a current Better Auth production user. Cal.com retains weight as Drizzle production cite + Next.js App Router migration cite + RSC composition pattern reference. Deel.com cite stands as surviving Better Auth indie/SMB production reference in this entry's Q3 set.
- **Cockpit /design pass complete.** All 4 cockpit decision entries vaulted (cockpit-shape + first-run-journey + auth-surface-mount + admin-list-endpoints-contract + file-structure) + 2 cockpit research entries (cockpit-stack-integration-research + admin-list-endpoints-research + cockpit-folder-structure-research) + 2 amendments (`[[backend-stack]]` §7 + `[[backend-stack-research]]` Source 8) + amendments-in-place to `[[admin-auth-surface]]` from earlier in the day.
- **Cascade obligations queued for slice 8.3 implementation across all cockpit entries** (consolidated):
  - **`packages/auth-config/src/index.ts`** statement schema extension: `project: ['create', 'read'], apiKey: ['create', 'revoke'], org: ['read']` added to `statements` + role permission maps per `[[admin-list-endpoints-contract]]` cascade #1.
  - **`apps/backend/src/index.ts`** mount Better Auth handler: `app.on(['POST', 'GET'], '/api/auth/*', (c) => auth.handler(c.req.raw))` per `[[auth-surface-mount]]` (H).
  - **`apps/backend/src/projects/` (NEW module)** — 6 handlers per `[[admin-list-endpoints-contract]]`: `getOrgMeHandler`, `listProjectsHandler`, `createProjectHandler`, `getProjectHandler`, `createApiKeyHandler`, `revokeApiKeyHandler`. Each behind `adminGate` per `[[admin-auth-surface]]`. OTel spans per handler.
  - **Drizzle schema verification** — `projects.name`, `(organization_id, slug)` unique, `api_keys.name`, `api_keys.revoked_at` columns per `[[admin-list-endpoints-contract]]` cascade #3.
  - **`apps/cockpit/package.json`** — Next.js 16.2.6 + React 19.2 + Better Auth + @bokchoy/* workspace + TanStack Query + Tailwind v4 + Radix UI + RHF + Zod + shadcn deps per `[[file-structure]]` cascade #1.
  - **`apps/cockpit/next.config.ts`** — env-driven async rewrites per `[[auth-surface-mount]]` (V3) + Cache Components opt-in per `[[frontend-stack]]`.
  - **`apps/cockpit/tailwind.config.ts`** + **shadcn CLI bootstrap** to `components/ui/` per shadcn `components.json` convention.
  - **`apps/cockpit/app/layout.tsx`** — QueryClientProvider + Better Auth client provider.
  - **`apps/cockpit/components/layouts/{app-shell,sidebar,header,marketing-shell,auth-shell}.tsx`** — cross-feature shells per `[[file-structure]]`.
  - **`apps/cockpit/modules/{auth,projects,organizations,marketing}/`** vertical slices per `[[file-structure]]` + concrete contracts from `[[first-run-journey]]` (11-step) + `[[admin-list-endpoints-contract]]` (6 endpoints).
  - **`apps/cockpit/lib/{query-client,utils}.ts`** — query-client per `React.cache` per-request factory.
  - **Smoke script `/tmp/smoke-8-2.0-admin-gate.sh` extension** — covers the 6 new admin endpoints + (E2) two-POST flow + idempotency-key retry + cockpit OAuth flow end-to-end + Safari ITP verification per `[[cockpit-stack-integration-research]]` operational implications.
  - **PR-review enforcement** of F6 17-row anti-pattern table + Vercel skill rule applications + barrel-import discipline.
- **Vault count: 76 entries.** Two new entries (`cockpit-folder-structure-research` + `cockpit/file-structure`) + 2 amendments (`backend-stack` §7 + `backend-stack-research` Source 8) in this turn. — 4 production cockpits triangulated, NO convergent feature-folder pattern found; (P-LAYER) layer-based has 2/4 cites majority over (P-CAL) vertical-slice 1/4 + (P-DUB) horizontal-split 1/4; /design picks the path with named winning conditions)
- **`[[cockpit-folder-structure-research]]` LANDED 2026-05-11.** 4 production B2B SaaS cockpits source-walked: Cal.com (Next.js, `apps/web/modules/{feature}/` vertical-slice), Dub.co (Next.js, `apps/web/ui/{feature}/` + `lib/{feature}/` horizontal split), Documenso (Remix, `apps/remix/app/{layers}/` layer-based), Trigger.dev (Remix, `apps/webapp/app/{layers}/` layer-based). 5 findings + 2 conflicts surfaced.
- **F1 LOAD-BEARING: NO convergent production pattern across 4 cites.** 50% (2/4 — Documenso + Trigger.dev) ship LAYER-BASED at app-level. 25% (Cal.com 1/4) vertical-slice. 25% (Dub.co 1/4) horizontal split-by-feature. **Prior /design turn's "feature-folder is the production-cited norm" claim is FALSIFIED.**
- **F2:** Bulletproof React (alan2207/bulletproof-react ~16k stars) is tier-4 educational advocacy, NOT a production cite. 0/4 production cites ship strict Bulletproof shape. **Walked back the prior cite-weight.**
- **F3:** workspace-level feature/domain extraction is 2/4 partial convergence (Cal.com + Documenso) — correlates with cross-app reuse expectation. For BokChoy single-cockpit-app MVP: workspace extraction is over-engineering.
- **F4 tradeoff matrix** between feature-folder and layer-based — neither universally better; cuts both ways across cohesion/onboarding/refactoring/cross-feature-primitives/empty-modules-at-MVP axes.
- **F5:** Dub.co horizontal split-by-feature is a distinct 3rd path matching TS+React stacks with TanStack Query + RSC data/UI architectural separation. Documented as (P-DUB).
- **Walkback notes** for prior /design turns:
  - 2026-05-11 prior turn cited Bulletproof React as production-cited × 1. Reality: tier-4 advocacy. Walked back.
  - Prior turn's Cal.com-only single-cite was below *Industry-standard* gate floor (≥2 production systems). User correctly enforced the floor; 4-cite triangulation closes the gap.
  - Same correction-class as prior research walkbacks (Stripe `unit_amount_decimal` adjacency; Cal.com Better Auth misattribution; "split-deploy not documented" finding falsified by `concepts/cookies.mdx` page). **Third or fourth time this session I defaulted to defending-on-training-data + 1 cite; user enforced ≥2-cite floor.** Pattern worth flagging for future seats: position derivation must reach the production-cite floor BEFORE vault.
- **Three honest paths for `[[cockpit/file-structure]]` /design pick:**
  - **(P-LAYER)** Trigger.dev/Documenso style — `apps/cockpit/{components,hooks,services,utils,...}/` layer-based. 2/4 production cites (majority). Lowest-LOC + simplest at small scale; risk of dir-bloat past ~10 features.
  - **(P-CAL)** Cal.com vertical-slice — `apps/cockpit/modules/{feature}/{components,hooks,api,*-view.tsx}/`. 1/4 production cite. High feature cohesion; some empty modules at MVP scale.
  - **(P-DUB)** Dub.co horizontal split — `apps/cockpit/ui/{feature}/` + `apps/cockpit/lib/{feature}/`. 1/4 production cite. Clean data/UI separation matching `[[cockpit-stack-integration-research]]` RSC + TanStack Query architecture.
- **Open design forks owed to next /design pass:**
  - **(P-LAYER) / (P-CAL) / (P-DUB) pick** for `[[cockpit/file-structure]]`. (P-LAYER) has the cite majority; recommendation depends on whether short-term cite-floor weight beats long-term scaling forecast (10+ features post months-4-5 cockpit-core).
  - **2 mechanical amendments still queued** from prior /design pass: `[[backend-stack]]` §7 line 116 OAuth-primary shift + `[[backend-stack-research]]` Source 8 Cal.com cite-class amendment.
- **Vault count: 75.** New entry under `## architecture`.
- **`[[cockpit/admin-list-endpoints-contract]]` LANDED 2026-05-11.** Six endpoint contracts vaulted with all 6 convention picks ratified: (R2) bare success + Stripe-wrapped errors; (Pa-none) no pagination at MVP; (A1) nested apiKeys[] on project detail; (E2) split-POST with Idempotency-Key retry per `[[idempotency-strategy]]` D2-α; (Disc-no) no `object` discriminator field; (Pref-raw) raw UUIDs. (E1) bundling rejected on scope-mismatch with `[[wallet-http-contract]]` G5 customer-profile-axis precedent (G5 was wire-shape per-call; bundling is request-flow once-per-customer). 6 rejected alternatives + 3 failure modes with mitigations + 6 revisit-when triggers + 7 cascade obligations for slice 8.3 + 8.4 implementation.
- **All 4 cockpit /design entries shipped.** cockpit-shape + first-run-journey + auth-surface-mount + admin-list-endpoints-contract. Plus 2 amended-in-place entries (cockpit-stack-integration-research with Vercel-skills amendment + admin-list-endpoints-research as the source for the contract entry).
- **Remaining /design pass work (2 mechanical amendments queued):**
  - **`[[backend-stack]]` §7 line 116 amendment** — change cockpit web login from "email/password + magic-link (or OAuth on demand)" to "OAuth Google/GitHub primary + email/password fallback; magic-link deferred" per design pick (iii) 2026-05-11.
  - **`[[backend-stack-research]]` Source 8 cite-class amendment** — down-rate Cal.com cite-class for Better Auth claim per `[[cockpit-stack-integration-research]]` C2 (Cal.com on `next-auth 4.24.13`, not Better Auth); retain as RSC composition reference (auth-lib-agnostic patterns).
- **Slice 8.3 + 8.4 implementation cascade fully grounded.** All design decisions for cockpit β-shape + dashboard-v1-first-extensions vaulted. Implementation can quote contracts verbatim.
- **`[[cockpit/admin-list-endpoints-research]]` LANDED 2026-05-11.** 4-sub-question query (a/b/c/d) triangulated across Stripe + Vercel + Resend docs. 6 findings + 2 conflicts surfaced. **F1:** bare-data scalar envelopes production-cited × 3 — confirms (R2) bare success per `[[wallet-http-contract]]` G5. (R1) wrap REJECTED. **F2:** list envelope diverges; bare arrays at MVP scale + Stripe `{ object: 'list', data, has_more, url }` envelope post-pagination. **F3:** cursor pagination via `starting_after`/`ending_before` is Stripe canonical; no `total_count`. **F4:** nested sub-resource arrays on scalar detail endpoints production-cited × 2 (Vercel + Stripe) — confirms (A1) nested `apiKeys[]` on `GET /v1/projects/{id}`. **F5 LOAD-BEARING CONTRADICTION:** create-with-side-effect bundling NOT production-cited × 0/3 — Stripe / Vercel / Resend all split parent + child creation. **Walks back prior /design (E1) bundling pick.** Two paths owed: (E2) production-canonical split-POST OR (E1) BokChoy-specific bundling defensible on customer-profile-axis precedent from `[[wallet-http-contract]]` G5 + `[[wrapper-shape]]` Fork 2. **F6:** `object` discriminator non-convergent — recommend NO field (2-of-3 majority + internal-consistency).
- **Walkback notes** documented: prior /design seat's (E1) bundling pick was inferred-as-defensible without production cite; /research surfaces it as BokChoy-specific divergence. Same correction-class as prior /research walkbacks (Stripe `unit_amount_decimal` adjacency 2026-05-11; Cal.com Better Auth misattribution 2026-05-11).
- **Open design forks owed to next /design pass:**
  - **(E1) vs (E2)** on `POST /v1/projects` — the only true fork. (E2) is production-canonical safer path with named cost (race window + recovery surface). (E1) is customer-profile-axis-justified BokChoy divergence with named win-conditions (onboarding-confidence single round-trip + K1 visible-once preserved + matches existing BokChoy-divergence precedents).
  - **Resource ID prefix convention** — Stripe-style `proj_<UUID>` vs raw UUID (current BokChoy slice 8.1+ pattern). Internal-consistency favors raw UUID; Stripe-style adds machine-readable type tag.
  - **`object` discriminator field decision provisional NO** — revisit if SDK strategy requires runtime type tags.
- **`[[cockpit/auth-surface-mount]]` LANDED 2026-05-11.** Three concrete contracts vaulted: (V3) env-driven async `rewrites()` in cockpit `next.config.ts` proxying `/api/auth/:path*` + `/v1/:path*` to `BOKCHOY_BACKEND_URL`; (H) backend `app.on(['POST', 'GET'], '/api/auth/*', (c) => auth.handler(c.req.raw))` mount per `[[cockpit-stack-integration-research]]` F3 verbatim, NO `cors()` middleware under (P1) reverse-proxy; (T-multi) `trustedOrigins` array with prod + www + localhost + Vercel-preview wildcard. (P) OAuth provider config shape: `socialProviders: { google, github }` env-driven. (CK) cookie config: Better Auth defaults `SameSite=Lax` + `Secure=true`, no `crossSubDomainCookies` under (P1). (CL) cockpit `lib/auth-client.ts` with `baseURL: '/'` + `organizationClient()` plugin. 5 rejected alternatives + 3 failure modes (Vercel rewrite host-header bug; OAuth preview-deploy URL mismatch; env var typo) with mitigations + 6 revisit-when triggers + 1 inline /design decision (hardcoded `trustedOrigins` module-level constant matches D3 static-AC pattern from `[[admin-auth-surface]]`).
- **`[[cockpit/first-run-journey]]` LANDED 2026-05-11.** 11-step concrete sequence: marketing (step 0, force-static at `bokchoy.com/`) → OAuth (step 1-2) → auto-org-create via Better Auth org plugin (step 3) → projects empty-state (step 4) → create-project form (step 5-6) → API key visible-once modal (step 7) → copy (step 8) → user-side SDK install + first call (out-of-band steps 9-10) → cockpit polls `api_keys.last_used_at` for verification confirmation within ~5s (step 11). (J1) + (MK-COLOC) + (O1) + (K1) + (T2) all ratified. 5 rejected alternatives with named winning conditions; 3 failure modes with mitigations; 6 revisit-when triggers.
- **`[[cockpit/cockpit-shape]]` amended in same turn.** Marketing/landing colocated in cockpit Next.js app at `apps/cockpit/app/(marketing)/page.tsx` (force-static, served at apex `bokchoy.com/`). Production-ref-split-pattern (Stripe/Vercel/Linear/Resend separate marketing domains) walked back — those are mature-scale; BokChoy solo-dev MVP keeps marketing colocated. 3 revisit-when triggers added for splitting to separate workspace (marketing scope > 1 page; marketing-team hire; performance regression).
- **Domain strategy now implied across `[[cockpit-shape]]` + `[[first-run-journey]]`:** `bokchoy.com/` apex = cockpit Next.js app (marketing route at `/` via route group + auth-gated cockpit routes); `api.bokchoy.com/` = backend Hono. Vercel rewrites in cockpit's `vercel.json` proxy `/api/auth/*` + `/v1/*` to backend per `[[cockpit-stack-integration-research]]` F1 (reverse-proxy P1 pattern).
- **`[[cockpit/cockpit-shape]]` LANDED 2026-05-11.** All 5 sub-decisions ratified by user with full position derivation on file. (M2) dashboard v1 scope per `[[mvp-feature-sequence]]` months 1-3 + (L1) cockpit-creates-projects + (I1) left sidebar nav + slice sequencing (8.3 → 8.4 → 8.5 → 8.6) + 3-tier success criteria + 10-row non-goals table. 5 rejected alternatives with named winning conditions; 3 failure modes with mitigations; 6 revisit-when triggers.
- **Slice sequencing within months 1-3 dashboard v1:**
  - **8.3 (β):** sign-in OAuth + projects list + bootstrap-reason-codes action. ~3-4 cockpit pages + 3 backend endpoints (Better Auth handler mount + `GET /v1/orgs/me` + `GET /v1/projects`).
  - **8.4:** project creation flow + API-key issuance. Adds `POST /v1/projects` + `POST /v1/projects/{id}/api-keys` + `DELETE /v1/projects/{id}/api-keys/{keyId}`.
  - **8.5:** catalog editor (CRUD per `[[catalog-versioning]]` M2 stored-functions).
  - **8.6:** transaction inspector + player search.
- **Cascade obligations queued for slice 8.3 implementation** (carry from `[[cockpit-stack-integration-research]]` + `[[cockpit-shape]]`):
  - `apps/cockpit/vercel.json` rewrites
  - `apps/backend/src/index.ts` Better Auth handler mount
  - `packages/auth-config/src/index.ts` trustedOrigins extension
  - `apps/cockpit/package.json` deps (Next.js 16 + React 19 + Better Auth + TanStack Query + Tailwind v4 + Radix + RHF + Zod + shadcn)
  - `apps/cockpit/lib/auth-client.ts` Better Auth React client
  - `apps/cockpit/lib/session.ts` RSC session-read with `React.cache`
  - `apps/cockpit/app/sign-in/page.tsx` OAuth-primary + email/password fallback
  - `apps/cockpit/app/projects/page.tsx` projects list with section-level Suspense
  - `apps/cockpit/components/layout/sidebar.tsx` with `sections` prop accepting collapsible groups
  - `apps/backend/src/wallet/index.ts` or new module — `GET /v1/orgs/me` + `GET /v1/projects` admin endpoints behind `adminGate`
  - PR-review enforcement of F6 17-row anti-pattern table
- **Remaining cockpit /design entries queued (this session):**
  - **`[[cockpit/first-run-journey]]`** — concrete sign-up → org → project → bootstrap → first-SDK-call sequence; names cockpit-driven vs out-of-band steps.
  - **`[[cockpit/auth-surface-mount]]`** — concrete mount config per `[[cockpit-stack-integration-research]]` F1+F3+F7 (vercel.json + backend Hono mount + trustedOrigins + OAuth provider config sketch).
  - **`[[cockpit/admin-list-endpoints-contract]]`** — `GET /v1/orgs/me` + `GET /v1/projects` (+ `POST /v1/projects` per L1 pick) response schemas, pagination posture.
- **Mechanical amendments queued (this session, parallel):**
  - **`[[backend-stack]]` §7 line 116** — amend cockpit web login from "email/password + magic-link (or OAuth on demand)" to "OAuth Google/GitHub primary + email/password fallback; magic-link deferred" per design pick (iii) 2026-05-11.
  - **`[[backend-stack-research]]` Source 8** — down-rate Cal.com cite-class for Better Auth claim per `[[cockpit-stack-integration-research]]` C2 (Cal.com on `next-auth 4.24.13`, not Better Auth); retain as RSC composition reference.
- **Next on resume:** continue /design pass with `[[cockpit/first-run-journey]]` as the next decision. Dependency order: first-run-journey → auth-surface-mount → admin-list-endpoints-contract.

- **Mode (prior):** research → handoff to /design 2026-05-11 (cockpit-stack-integration research LANDED for slice 8.3 β-shape; 7 sources triangulated; reverse-proxy split-deploy pattern + Better Auth+Hono mount + RSC composition + 12-row anti-pattern table vaulted; 2 contradictions surfaced including Cal.com cite-class amendment owed to `[[backend-stack-research]]`)
- **/research pass 2026-05-11 LANDED.** One entry shipped covering slice 8.3 (cockpit β-shape per `[[admin-auth-surface]]` *Mitigations* row 4 amendment) two-axis query merged (Q-A) auth+split-deploy + (Q-D) RSC composition + anti-pattern audit. Triangulated ✓ — Better Auth demo Next.js source-walked at commit `e21d744` + Better Auth Hono integration docs + cookies/concepts docs + Next.js integration docs + Next.js 16.2.6 docs (lastUpdated 2026-05-07) + Cal.com production code source-walked at commit `fb01494` + TanStack Start migration cite (reused contradiction probe).
- **`[[cockpit-stack-integration-research]]`** — slice 8.3 architecture grounded. F1-F7 + 12-row anti-pattern table + 8 operational implications enumerated.
- **F1 (LOAD-BEARING): reverse-proxy split-deploy (P1) is the production-cited pattern.** Cockpit `vercel.json` `rewrites` block proxies `/api/auth/:path*` + `/v1/:path*` to `https://api.bokchoy.com/:path*`. Browser sees first-party requests; cookies work without SameSite=None/Safari-ITP complexity. Production-cited via Better Auth `concepts/cookies.mdx` Vercel example verbatim + Better Auth `integrations/hono.mdx` `auth.handler(c.req.raw)` mount.
- **F2:** shared-parent-domain (P2) rejected for BokChoy — wins only when infrastructure forces distinct hosts.
- **F3 (LOAD-BEARING):** Better Auth+Hono mount canonical 3 LOC: `app.on(["POST", "GET"], "/api/auth/*", (c) => auth.handler(c.req.raw))`. App-level session-injection middleware from Source 1 NOT adopted — adminGate per-route call from slice 8.2.0 is already authoritative; adding app-level injection would double the session lookup.
- **F4:** cockpit has NO Better Auth INSTANCE — instance + DB connection live only on backend. Cockpit uses `better-auth/react` CLIENT only.
- **F5 RSC composition canonical patterns:** parallel independent fetches via `Promise.all` (Cal.com pervasive); sequential dependent fetches with downstream `<Suspense>` (Next.js docs explicit-acceptable); `<Suspense>` granular > `loading.tsx` route-level (Next.js docs recommendation); layout uncached-data BLOCKS navigation; Client Component data via `use(promise)` API; `React.cache` for cross-component memoization within request scope.
- **F6 12-row anti-pattern table with named fixes** — extends `[[frontend-stack]]` + `[[tanstack-query-rsc-research]]` Q4 enforcement list: 1 cascaded `'use client'`; 2 sequential awaits for independent data; 3 layout uncached without local Suspense; 4 page-level Suspense for partially-slow content; 5 `getSessionCookie` for authz; 6 `fetchQuery` SSR; 7 staleTime=0; 8 double-invalidate; 9 TanStack Query owning all fetching; 10 Server Action signIn without `nextCookies()`; 11 `'use client'` on outer layout; 12 foreign-domain cookies without proxy/attribute config.
- **F7 OAuth-primary sign-in form pattern:** Client Component buttons calling `authClient.signIn.social({provider: 'google'/'github'})`; email/password fallback via small form (RHF + Zod + shadcn); no magic-link at MVP per design pick (iii) 2026-05-11.
- **C1 contradiction walked back:** earlier WebFetch "Better Auth split-deploy not documented" finding falsified — was looking at the Next.js integration page only; split-deploy lives in `concepts/cookies.mdx` + `integrations/hono.mdx`. Both pages explicitly document patterns. Walking back.
- **C2 cite-class amendment owed to `[[backend-stack-research]]` Source 8.** Cal.com listed there as Better Auth production user; reality (Source 6 source-walk) is `next-auth 4.24.13` — Auth.js security-patch-only mode, Better Auth migration target. Same correction-class as Stripe `unit_amount_decimal` adjacency from prior /design seat. Cal.com retains weight as RSC composition reference (auth-lib-agnostic patterns); does NOT validate Better Auth claims.
- **C3 TanStack Start migration cite (reused contradiction).** Per `[[frontend-stack-research]]`: RSC contested at migration edge; defensible at greenfield MVP scale. Not novel to this entry.
- **Amendment 2026-05-11 — Vercel skills integrated** per user directive. Sources 8 + 9 added: `vercel-react-best-practices` (69 rules, 8 priorities; Eliminating Waterfalls CRITICAL/2-10× per `async-parallel.md`) + `vercel-composition-patterns` (4 priorities). F8 surfaces 5 NEW anti-patterns extending F6 table (rows 13-17): server-no-shared-module-state (HIGH, request-data leaks); server-dedup-props (LOW-HIGH by type, RSC→client serialization breaks on new refs); bundle-barrel-imports (CRITICAL, tree-shaking blocker); architecture-avoid-boolean-props (MEDIUM, composition over prop proliferation); react19-no-forwardref (LOW, React 19 obsolete pattern). F9-F10 concrete enforcement: `apps/cockpit/lib/session.ts` MUST use `React.cache` per-request only; shadcn imports MUST be direct-path not barrel; component API design via composition from day 1; React 19 `use(Context)` over `useContext()`.
- **Anti-default applied:** searched for production cite explicitly REJECTING reverse-proxy in favor of `SameSite=None`. None found in surveyed material. Better Auth docs recommend `SameSite=Lax` + reverse-proxy as the safer Safari-ITP-compatible path.
- **Operational implications for slice 8.3 (10 cascade obligations enumerated):**
  1. `apps/cockpit/vercel.json` — `rewrites` block proxying `/api/auth/:path*` + `/v1/:path*` to backend.
  2. `apps/backend/src/index.ts` — mount Better Auth via `app.on(["POST", "GET"], "/api/auth/*", (c) => auth.handler(c.req.raw))`.
  3. `packages/auth-config/src/index.ts` — `trustedOrigins` extension with cockpit prod + dev origins.
  4. `apps/cockpit/package.json` — Next.js 16.2.6 + React 19.2 + Better Auth + @bokchoy/auth-config workspace + TanStack Query + Tailwind v4 + Radix + RHF + Zod + shadcn deps.
  5. `apps/cockpit/lib/auth-client.ts` — Better Auth React client with `baseURL: '/'` (Vercel rewrite handles backend).
  6. `apps/cockpit/lib/session.ts` — RSC session-read via `React.cache(fetch '/api/auth/get-session')`.
  7. `apps/cockpit/app/sign-in/page.tsx` — OAuth-primary buttons + email/password fallback.
  8. `apps/cockpit/app/projects/page.tsx` — projects list with section-level `<Suspense>` composition.
  9. `apps/backend/src/wallet/index.ts` or new module — `GET /v1/orgs/me` + `GET /v1/projects` admin endpoints behind `adminGate`.
  10. PR-review anti-pattern enforcement: F6 12-row table extension to existing enforcement.
- **Vault amendments owed to next /design pass:**
  - `[[backend-stack]]` §7 line 116: amend cockpit web login from "email/password + magic-link (or OAuth on demand)" to "OAuth Google/GitHub primary + email/password fallback; magic-link deferred."
  - `[[backend-stack-research]]` Source 8: down-rate Cal.com cite-class for Better Auth claim (retains weight as RSC composition reference).
- **Open threads (carried, NOT blocking implementation):**
  - Safari ITP production verification owed — slice 8.3 smoke must test OAuth callback flow + session persistence in Safari specifically.
  - OAuth provider config (Google + GitHub client IDs / secrets / callback URLs / scopes) — operational config, not research.
  - `nextCookies()` plugin decision deferred — needed only if cockpit adopts Server Actions for sign-in post-MVP. Current F7 pattern uses Client Component sign-in only.
- **Next on resume — recommended sequence:**
  1. **Switch to `/design`** with `[[cockpit-stack-integration-research]]` in hand. /design produces:
     - `[[cockpit/cockpit-shape]]` — MVP cockpit scope (β slice scope confirmed + dashboard v1 vs cockpit-core sequencing per `[[mvp-feature-sequence]]`).
     - `[[cockpit/first-run-journey]]` — sign-up → org → project → bootstrap → API key journey.
     - `[[cockpit/auth-surface-mount]]` — concrete mount shape per F1+F3+F7.
     - Possibly `[[cockpit/admin-list-endpoints-contract]]` — `GET /v1/orgs/me` + `GET /v1/projects` shapes.
     - Plus the two `[[backend-stack]]` amendments (auth UX + Cal.com cite-class).
  2. After /design pass → /implementation can quote contracts and ship slice 8.3.
  3. Working tree as of this session = committed slice 8.1.x + 8.2.0 + 8.2.1 cluster + 2 follow-up commits + this cockpit-stack-integration research entry + state.md checkpoint (uncommitted).

- **Mode (prior):** implementation → slice 8.2.1 FULLY LANDED 2026-05-11 (`bootstrapProjectReasonCodes` consumer + integration smoke 11/11 passing + owner-role gap resolved per (a) + .env.example updated + bearer transport plugin enabled)
