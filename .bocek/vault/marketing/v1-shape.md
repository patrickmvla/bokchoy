...
10. **`apps/cockpit/modules/marketing/components/code-walkthrough.tsx`** — code walk-through section. Shows 3-4 sibling SDK operations (credit + debit + balance + history) with the same friendly-name register as the hero. Static syntax-highlighting via shiki (build-time) — no client-side syntax highlighter to keep bundle weight off the marketing route.
    ...
    **LANDED 2026-05-18 in two stages.** M-5 (2026-05-17) shipped credit + debit live and balance + history as `// Coming soon` muted snippets — the read-surface wire-row shape was indeterminate when this cascade was written (2026-05-14) and the M-5 seat deferred the live shapes per user-ratified Option B. GAP 11 (2026-05-18) shipped the read surface via `[[wallet/balance-history-contract]]` (the wire-row shape question resolved by `[[wallet/.research/per-row-running-balance-research]]` audience-class verdict — game-economy class omits per-row `balanceAfter`; cursor pagination on `transactions.id`; stateless reads with synthesized zero/empty on unknown). Same /implementation seat flipped `code-walkthrough.tsx` from "Coming soon" to live snippets matching the shipped SDK shape. All 4 operations now render as live `@bokchoy/sdk-node` calls; "Coming soon" staging is retired from the marketing apex.
