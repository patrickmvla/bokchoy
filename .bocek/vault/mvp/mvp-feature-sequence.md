...
|---|---|---|
| **Day 17 → end Month 2 (~43 days)** | **Inventory ✓ LANDED 2026-05-19** (`[[inventory/inventory-contract]]` shipped per M-A item model — items + inventory tables + 2 SQL functions + 4 wrappers in new `@bokchoy/inventory` workspace + 4 backend routes + SDK extension `bokchoy.inventory.*` with 4 methods + 3 error classes + 8 unit tests + 7 smoke tests; three gates green through all 5 implementation slices). **Simple Shop ✓ LANDED 2026-05-21** (`[[shop/shop-contract]]` shipped per B1b offer-as-priced-unit — `offers`/`offer_prices`/`offer_items` tables + RLS in migration 0013 + atomic `purchase_offer_by_external_id` SQL fn composing the existing `wallet_debit`+`item_grant` (1 debit + N grants, rollback-on-any-failure) + 3 wrappers in new `@bokchoy/shop` workspace + 3 backend routes (`POST /v1/players/{externalId}/purchases` + `GET /v1/offers` + `GET /v1/offers/{offerCode}`) + SDK `bokchoy.shop.*` with 3 methods + 3 error classes (BC090/091/092) + `InsufficientFundsError` (BC010 now surfaces as friendly `INSUFFICIENT_FUNDS` across `/debit` + `/purchases`) + 9 SDK unit tests + 8 smoke tests; three gates green through 5 implementation slices). Remaining: IAP fulfillment (Apple/Google receipt validation via `staged_jobs` worker; `iap_receipts` table already exists) + Dashboard v1 (catalog/offer editor + transaction inspector + player search in `apps/cockpit/modules/`). | 4 primitives + SDK + dashboard (2 of 4 primitives LANDED) |
