...
**ERRATUM 2026-05-11 (cite-class amendment):** Cal.com is NOT a current Better Auth production user. Source-walked at commit `fb01494` (per `[[cockpit-stack-integration-research]]` C2 + `[[cockpit-folder-structure-research]]` Source 1): `apps/web/package.json:111` lists `next-auth: 4.24.13` — Auth.js v4 (security-patch-only mode per Auth.js team's 2025-09-22 transition to Better Auth absorption, also noted in this entry's Source 7). Better Auth founder's HN cite likely referred to Cal.com as an Auth.js user that Better Auth's ecosystem absorption affects, NOT a current Better Auth user. **Cal.com is a Better Auth MIGRATION TARGET, not a current Better Auth deployment.** Same correction-class as the prior /design seat's Stripe `unit_amount_decimal` adjacency cite (field-class mismatched). Cal.com retains weight as: (a) a Drizzle production cite (per `apps/web/package.json` + this entry's Source 8 testimony on database delegation patterns, which Cal.com does via its own architecture not specifically through Better Auth); (b) a Next.js App Router migration production cite (250k+ LOC migration per `[[frontend-stack-research]]`); (c) an RSC composition pattern reference (auth-lib-agnostic patterns documented in `[[cockpit-stack-integration-research]]` Source 6 + `[[cockpit-folder-structure-research]]` Source 1). Does NOT validate Better Auth claims; Deel.com cite stands as the surviving Better Auth indie/SMB production reference in this entry's Q3 set.
