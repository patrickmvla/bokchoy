...
- **Concurrency:** two requests with the same key arriving milliseconds apart: the first to acquire the row lock proceeds; the second sees `locked_at` recent and returns 409 with `bokchoy_idempotency_key_in_use`. Lock timeout is configurable (initial value 30 seconds); after timeout, a stuck row is treated as recoverable and re-locked. **No 5-second client-wait pattern** — the prior version's "wait up to 5 seconds then return 409" was unsourced and added unnecessary client-side complexity. **Canonical race-loser implementation** (added 2026-05-11 per slice 8.1.6 verification): the SELECT-then-INSERT race window where two transactions both see "no row" closes via Postgres native upsert: `INSERT INTO idempotency_keys (...) VALUES (...) ON CONFLICT (project_id, idempotency_key) DO NOTHING RETURNING id`. On empty `RETURNING` (winner committed first), middleware re-SELECTs the same row inside the same `withTenant` transaction and dispatches a TRIMMED state machine — body-mismatch → 422 BC002, completed_at-set + body-match → 200 REPLAY, else → 409 BC001. The lock-expired re-lock branch is intentionally skipped on the race-loser path: the winner just INSERTed milliseconds ago so `locked_at` is fresh by construction; re-locking would race the winner and could double-execute the handler when both transactions write their completion UPDATEs. (docs-cited / high — Postgres ON CONFLICT clause docs §6.4 + Drizzle `.onConflictDoNothing().returning()` API. production-cited / medium — Brandur `rocket-rides-atomic@94b370d` schema's `locked_at TIMESTAMPTZ DEFAULT now()` shape implies the upsert pattern at the app layer; app code not source-walked. production-cited / high (this project) — `apps/backend/src/idempotency/middleware.ts:137-200` verified empirically: 10× xargs -P concurrent same-key resolved 1×200 + 9×409 + 0×500; 10× mixed-body resolved 1×200 + 4×409 + 5×422 BC002 + 0×500.) **Generalization note:** the ON CONFLICT DO NOTHING + re-SELECT shape is the canonical race-loser pattern for any cross-cutting middleware writing to a UNIQUE-constrained table. Future consumers (audit-log, outbox, DSR queue, rate-limit bucket writers) should re-use this shape; if/when a second consumer ships, promote the pattern to its own `[[upsert-race-loser-pattern]]` entry. Until then it lives here, scoped to idempotency.
