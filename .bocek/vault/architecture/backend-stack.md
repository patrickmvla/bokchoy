...
2. **Secondary — type-level discipline at the Db API surface.** Concrete pattern TBD at first feature schema; candidate: branded tx types where RLS-protected table accessors require a `TenantTx` brand obtainable only from inside a `withTenant(...)` callback. Queries against RLS-protected tables outside `withTenant` fail at compile time, not runtime. Open thread — design follow-up triggered by first RLS-protected feature schema landing.
