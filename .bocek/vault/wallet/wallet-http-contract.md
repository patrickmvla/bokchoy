...
| `purchaseOfferByExternalId` | `POST /v1/players/{playerExternalId}/purchases` | SDK API key | Shop |
| `listOffers` | `GET /v1/offers` | SDK API key | Shop |
| `getOffer` | `GET /v1/offers/{offerCode}` | SDK API key | Shop |

The Shop routes register per `[[shop/shop-contract]]` cascade #10. `POST /v1/players/{externalId}/purchases` carries the full mutating auth chain (`apiKeyMiddleware` + `idempotencyMiddleware` + `sValidator('param','json')`); the two `GET /v1/offers...` are project-scoped catalog reads (apiKey only, no idempotency — same Stripe GET convention as the GAP-11 reads). BC010 now surfaces as friendly `INSUFFICIENT_FUNDS` (422) from both `/debit` and `/purchases` via the `BC_FRIENDLY_CODE` map in `error-middleware.ts`. (NOTE: the 4 inventory routes from `[[inventory/inventory-contract]]` are live in `apps/backend/src/inventory/index.ts` but were never back-registered in this table — pre-existing bookkeeping gap, not introduced by Shop.)
...
**(X) Stripe-wrapped:** `{ "error": { "code": "INSUFFICIENT_FUNDS", "message": "Insufficient funds", "walletId": "...", "requested": "100.0000", "available": "50.0000" } }`. The `BcError.details` fields are flattened as siblings inside `error`; the wire `code` is the friendly code where one is mapped (see *Error-code-naming policy* below), else the raw `BcCode`.

**Error-code-naming policy (amended 2026-05-21 per `[[shop/contract-reconciliation-2026-05-21]]` D3 — records the Slice-4 BC010 change):** customer-facing BcCodes surface under a friendly UPPER_SNAKE wire code via one of two mechanisms — **(i) the global `BC_FRIENDLY_CODE` map** in `apps/backend/src/infra/error-middleware.ts`, for errors whose raw `details` fields are a sufficient body: `BC010 → INSUFFICIENT_FUNDS` (422, with `walletId`/`requested`/`available` flattened); **(ii) handler-local translation**, for errors needing a body the middleware can't assemble (enumeration or discriminated-variant dispatch): `UNKNOWN_CURRENCY` (BC060, `availableCodes`), `UNKNOWN_ITEM` (BC080, `availableItems`), `INVENTORY_OVERFLOW` (BC081), `INSUFFICIENT_INVENTORY` (BC082), `UNKNOWN_OFFER` (BC090), `OFFER_INACTIVE` (BC091), `INVALID_PAYMENT_CURRENCY` (BC092, `acceptedCurrencies`), `OFFER_MISCONFIGURED` (BC093). Raw `BCxxx` passes through for internal/rare codes customers don't dispatch on (BC020, BC040, BC050…). A new customer-facing error picks (i) if detail-fields suffice, (ii) if it needs a richer body — never a third mechanism. The SDK exposes `InsufficientFundsError` for `INSUFFICIENT_FUNDS`. NOTE: the BC010 detail amounts (`requested`/`available`) are NUMERIC strings on the wire, consistent with the shop `amount` and wallet `balance` string discipline.
